...
class EvolveWP_ERP_Notifications {

    private static $notification_types = array();

    public static function init() {
        self::register_notification_type('system_alert', __('System Alerts', 'evolvewp-predictiveerp'));
        self::register_notification_type('update_notice', __('Update Notices', 'evolvewp-predictiveerp'));
        
        do_action('evolvewp_erp_register_notification_types');
        
        add_action('evolvewp_erp_process_notifications', array(__CLASS__, 'process_pending_notifications'));
        
        if (!wp_next_scheduled('evolvewp_erp_process_notifications')) {
            wp_schedule_event(time(), 'hourly', 'evolvewp_erp_process_notifications');
        }
    }
    ...
    public static function create_notification($type, $message, $args = array()) {
        ...
        do_action('evolvewp_erp_notification_created', $notification_id, $type, $message, $args);
    ...
    private static function store_notification($type, $message, $args) {
        ...
        $table_name = $wpdb->prefix . 'evolvewp_erp_notifications';
    ...
    private static function send_email_notification($notification_id, $type, $message, $args) {
        ...
        $subject = sprintf(__('[%1$s] %2$s Notification', 'evolvewp-predictiveerp'), 
            get_bloginfo('name'), 
        ...
        $email_body = sprintf(
            __("Hello %1\$s,\n\nYou have received a new notification:\n\n%2\$s\n\nRegards,\n%3\$s Team", 'evolvewp-predictiveerp'),
            $user_name,
            $message,
            get_bloginfo('name')
        );
     ...
     *
     * Runs on the evolvewp_erp_process_notifications cron hook (hourly). A transient
    ...
    public static function process_pending_notifications() {
        ...
        $lock_key = 'evolvewp_erp_notifications_sweep_lock';
        if ( wp_cache_get( $lock_key, 'evolvewp_erp_notifications' ) ) {
            return;
        }
        ...
        $wpdb->query( $wpdb->prepare(
            'UPDATE ' . esc_sql( $wpdb->prefix . 'evolvewp_erp_notifications' ) . ' SET is_read = 1 WHERE expires_at IS NOT NULL AND expires_at < %s',
            current_time( 'mysql' )
        ) );

        // Lock for 55 minutes so the next scheduled run (hourly) can proceed.
        wp_cache_set( $lock_key, true, 'evolvewp_erp_notifications', 55 * MINUTE_IN_SECONDS );

        // Flush cached reads so the updated state is visible immediately.
        wp_cache_flush_group( 'evolvewp_erp_notifications' );

        do_action( 'evolvewp_erp_process_scheduled_notifications' );
    }
    ...
    public static function get_notifications($user_id = 0, $args = array()) {
        ...
        $cache_key = 'evolvewp_erp_notifications_' . md5(serialize(array($user_id, $args)));
        $notifications = wp_cache_get($cache_key, 'evolvewp_erp_notifications');
        ...
        if (false === $notifications) {
            ...
            $safe_table   = esc_sql( $wpdb->prefix . 'evolvewp_erp_notifications' );
            ...
            wp_cache_set( $cache_key, $notifications, 'evolvewp_erp_notifications', 300 ); // 5 min cache
        }
    ...
    public static function mark_as_read( $notification_id, $user_id = 0 ) {
        ...
        if ($updated !== false) {
            wp_cache_delete( 'evolvewp_erp_unread_count_' . $user_id, 'evolvewp_erp_notifications' );
            wp_cache_flush_group( 'evolvewp_erp_notifications' );
        }
    ...
    public static function maybe_create_tables() {
        ...
        // Check cache first
        $cache_key = 'evolvewp_erp_table_exists_' . $table_name;
        $table_exists = wp_cache_get($cache_key, 'evolvewp_erp_notifications');
        
        if (false === $table_exists) {
            $table_exists = $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name)) === $table_name;
            wp_cache_set($cache_key, $table_exists, 'evolvewp_erp_notifications', 3600);
        }
    ...
    public static function snooze_notification($notification_id, $duration = 3600) {
        ...
        $table = $wpdb->prefix . 'evolvewp_erp_notifications';
    ...
    public static function get_unread_count($user_id) {
        ...
        $cache_key = 'evolvewp_erp_unread_count_' . $user_id;
        $count = wp_cache_get($cache_key, 'evolvewp_erp_notifications');
        
        if (false === $count) {
            $table = $wpdb->prefix . 'evolvewp_erp_notifications';
            
            // Check if table exists first (with caching)
            $table_cache_key = 'evolvewp_erp_table_exists_' . $table;
            $table_exists = wp_cache_get($table_cache_key, 'evolvewp_erp_notifications');
            
            if (false === $table_exists) {
                $table_exists = $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table)) === $table;
                wp_cache_set($table_cache_key, $table_exists, 'evolvewp_erp_notifications', 3600);
            }
            ...
            $count = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}evolvewp_erp_notifications WHERE (user_id = %d OR user_id = 0) AND is_read = 0 AND (expires_at IS NULL OR expires_at > %s)",
                $user_id,
                current_time('mysql')
            ));
            
            wp_cache_set($cache_key, $count, 'evolvewp_erp_notifications', 60); // 1 min cache
        }
...
add_action('init', array('EvolveWP_ERP_Notifications', 'init'));
