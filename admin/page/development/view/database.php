<?php
/**
 * EvolveWP PredictiveERP Development - Database Diagnosis Tab
 *
...
class EvolveWP_ERP_Admin_Development_Database {
    ...
    public static function output() {
        ...
        <div class="evolvewp-predictiveerp-dev-section">
            <h2><?php esc_html_e('Database Information', 'evolvewp-predictiveerp'); ?></h2>
            
            <table class="widefat">
                <tbody>
                    <tr>
                        <th><?php esc_html_e('Database Name', 'evolvewp-predictiveerp'); ?></th>
                        <td><code><?php echo esc_html(DB_NAME); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Database Host', 'evolvewp-predictiveerp'); ?></th>
                        <td><code><?php echo esc_html(DB_HOST); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Table Prefix', 'evolvewp-predictiveerp'); ?></th>
                        <td><code><?php echo esc_html($wpdb->prefix); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Database Charset', 'evolvewp-predictiveerp'); ?></th>
                        <td><code><?php echo esc_html(DB_CHARSET); ?></code></td>
                    </tr>
                    <tr>
                        <th><?php esc_html_e('Database Collate', 'evolvewp-predictiveerp'); ?></th>
                        <td><code><?php echo esc_html(DB_COLLATE ? DB_COLLATE : 'default'); ?></code></td>
                    </tr>
                </tbody>
            </table>

            <h3><?php esc_html_e('WordPress Tables', 'evolvewp-predictiveerp'); ?></h3>
            <?php
            // SHOW TABLES has no WP API equivalent. Cached for 5 minutes so
            // repeated renders within the same request do not re-query.
            $tables_cache_key = 'evolvewp_erp_dev_tables_' . md5( $wpdb->prefix );
            $tables           = wp_cache_get( $tables_cache_key, 'evolvewp_erp_dev' );
            if ( false === $tables ) {
                ...
                wp_cache_set( $tables_cache_key, $tables, 'evolvewp_erp_dev', 5 * MINUTE_IN_SECONDS );
            }
            ?>
            <table class="widefat">
                <thead>
                    <tr>
                        <th><?php esc_html_e('Table Name', 'evolvewp-predictiveerp'); ?></th>
                        <th><?php esc_html_e('Rows', 'evolvewp-predictiveerp'); ?></th>
                        <th><?php esc_html_e('Size', 'evolvewp-predictiveerp'); ?></th>
                    </tr>
                </thead>
                <tbody>
                    <?php foreach ( $tables as $table ) :
                        ...
                        $count_cache_key = 'evolvewp_erp_dev_count_' . md5( $table_name );
                        $row_count       = wp_cache_get( $count_cache_key, 'evolvewp_erp_dev' );
                        if ( false === $row_count ) {
                            $row_count = $wpdb->get_var( 'SELECT COUNT(*) FROM `' . $safe_table . '`' );
                            wp_cache_set( $count_cache_key, $row_count, 'evolvewp_erp_dev', 5 * MINUTE_IN_SECONDS );
                        }

                        // SHOW TABLE STATUS has no WP API equivalent.
                        // Cached per table name for 5 minutes.
                        $status_cache_key = 'evolvewp_erp_dev_status_' . md5( $table_name );
                        $table_status     = wp_cache_get( $status_cache_key, 'evolvewp_erp_dev' );
                        if ( false === $table_status ) {
                            ...
                            wp_cache_set( $status_cache_key, $table_status, 'evolvewp_erp_dev', 5 * MINUTE_IN_SECONDS );
                        }
