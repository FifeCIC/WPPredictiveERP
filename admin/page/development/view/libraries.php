...
$monitor = EvolveWP_ERP_Library_Update_Monitor::instance();
$evolvewp_erp_libraries = $monitor->get_libraries();
$updates = array();

// Check for updates if requested
if ( isset( $_GET['check_updates'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'evolvewp_erp_check_library_updates' ) ) {
    $monitor->clear_cache();
...
// Get cached updates
foreach ( $evolvewp_erp_libraries as $slug => $library ) {
    $updates[ $slug ] = $monitor->check_updates( $slug );
}
?>

<div class="wrap evolvewp-predictiveerp-libraries-tab">
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <h2>Bundled Libraries</h2>
        <a href="<?php echo esc_url( add_query_arg( array( 'check_updates' => '1', '_wpnonce' => wp_create_nonce( 'evolvewp_erp_check_library_updates' ) ) ) ); ?>" class="button button-primary">
            <span class="dashicons dashicons-update" style="margin-top: 3px;"></span> Check for Updates
        </a>
    </div>
    
    <p>EvolveWP PredictiveERP bundles the following third-party libraries. Updates are checked automatically every 6 hours.</p>
    ...
    <table class="wp-list-table widefat fixed striped">
        ...
        <tbody>
            <?php foreach ( $evolvewp_erp_libraries as $slug => $library ) : 
                $update = $updates[ $slug ];
            ...
            <tr class="evolvewp-predictiveerp-release-notes" style="display: none;" data-library="<?php echo esc_attr( $slug ); ?>">
    ...
    <div style="margin-top: 30px; padding: 20px; background: #f0f6fc; border-left: 4px solid #0073aa;">
        ...
        <ul style="margin-bottom: 0;">
            <li><strong>Automatic Checking:</strong> EvolveWP PredictiveERP checks for updates every 6 hours via GitHub API</li>
...
<style>
.evolvewp-predictiveerp-libraries-tab .wp-list-table td {
    vertical-align: top;
    padding: 12px 10px;
}
.evolvewp-predictiveerp-libraries-tab .button-small {
    height: auto;
    padding: 4px 8px;
    font-size: 12px;
}
...
jQuery(document).ready(function($) {
    ...
    $(document).on('click', '.evolvewp-predictiveerp-dismiss-library-notice', function(e) {
        e.preventDefault();
        $.post(ajaxurl, {
            action: 'evolvewp_erp_dismiss_library_notice'
        });
