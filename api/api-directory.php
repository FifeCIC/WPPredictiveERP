<?php
/**
 * EvolveWP PredictiveERP API Directory — registry of all available connectors.
 *
 * ROLE: api-endpoint
 *
 ...
 *
 * CONSUMED BY:
 *   - EvolveWP_ERP_API_Factory (looks up providers to instantiate)
 ...
 * 2. Runtime — plugins call EvolveWP_ERP_API_Directory::register() or use the
 *    'evolvewp_erp_api_providers' filter during init.
 *
 * @since 1.0.0
 * @since 3.1.0 Added runtime registration, get_provider_capabilities(),
 *              get_configured_providers(), is_registered().
 */
class EvolveWP_ERP_API_Directory {
	...
	public static function get_all_providers() {
		 ...
		 */
		self::$cache = apply_filters( 'evolvewp_erp_api_providers', $providers );
	...
	public static function get_configured_providers() {
		...
		foreach ( $all as $provider_id => $provider ) {
			$key = get_option( "evolvewp_erp_api_{$provider_id}_key", '' );
			if ( ! empty( $key ) ) {
	...
	public static function get_provider_capabilities( $provider_id ) {
		$instance = EvolveWP_ERP_API_Factory::create( $provider_id );
	 ...
	 *
	 * These are boilerplate examples shipped with EvolveWP PredictiveERP. When cloning to
	...
	private static function get_default_providers() {
		return array(
			'custom_api' => array(
				...
				'class_name'  => 'EvolveWP_ERP_Custom_API',
				'auth_type'   => 'api_key',
				'icon'        => 'dashicons-admin-generic',
			...
			'discord'    => array(
				...
				'class_name'  => 'EvolveWP_ERP_Discord_API',
