<?php
/**
 * EvolveWP PredictiveERP API Factory — creates connector instances.
 *
 * ROLE: api-endpoint
 *
 ...
 *
 * DEPENDS ON:
 *   - EvolveWP_ERP_API_Directory (looks up provider metadata)
 *   - EvolveWP PredictiveERP\API\Connector_Interface (validates instances)
 *   - WordPress functions: get_option, is_wp_error
 *
 * CONSUMED BY:
 *   - evolvewp_erp_connector() global accessor in functions.php
 ...
 */
class EvolveWP_ERP_API_Factory {

	/**
	 * Cached connector instances keyed by "provider_id:account_id".
	 *
	 * @since 3.1.0
	 * @var   array<string, \EvolveWP\PredictiveERP\API\Connector_Interface>
	 */
	...
	public static function create( $provider_id, $args = array() ) {
		$provider = EvolveWP_ERP_API_Directory::get_provider( $provider_id );
		...
		if ( ! $provider ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Unknown API provider: %s', 'evolvewp-predictiveerp' ),
					$provider_id
				)
			);
		}
		...
		if ( ! empty( $provider['class_path'] ) && ! class_exists( $provider['class_name'], false ) ) {
			$class_file = EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'api/' . $provider['class_path'];
			...
			if ( ! file_exists( $class_file ) ) {
				return new \WP_Error(
					...
					sprintf(
						...
						__( 'Connector class file not found: %1$s (provider: %2$s)', 'evolvewp-predictiveerp' ),
						$provider['class_path'],
						$provider_id
					)
				);
		...
		if ( ! class_exists( $provider['class_name'] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Connector class not found: %1$s (provider: %2$s)', 'evolvewp-predictiveerp' ),
					$provider['class_name'],
					$provider_id
				)
			);
		...
		try {
			...
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Failed to create %1$s connector: %2$s', 'evolvewp-predictiveerp' ),
					$provider_id,
					$e->getMessage()
				)
			);
		...
		if ( ! ( $instance instanceof \EvolveWP\PredictiveERP\API\Connector_Interface ) ) {
			return new \WP_Error(
				'invalid_connector',
				sprintf(
					/* translators: 1: class name, 2: interface name */
					__( '%1$s must implement %2$s.', 'evolvewp-predictiveerp' ),
					$provider['class_name'],
	 ...
	 *   evolvewp_erp_api_{provider_id}_{field}           (default account)
	 *   evolvewp_erp_api_{provider_id}_{account_id}_{field}  (named account)
	 *
	 * @since  1.0.0
	...
	public static function create_from_settings( $provider_id, $account_id = '' ) {
		...
		$prefix = empty( $account_id )
			? "evolvewp_erp_api_{$provider_id}"
			: "evolvewp_erp_api_{$provider_id}_{$account_id}";
		 ...
		 */
		$args = apply_filters( 'evolvewp_erp_connector_credentials', $args, $provider_id, $account_id );
	 ...
	 * @since  3.1.0
	 *
	 * @return array<string, \EvolveWP\PredictiveERP\API\Connector_Interface> Keyed by provider ID.
	 */
