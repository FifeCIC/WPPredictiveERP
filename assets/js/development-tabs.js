/**
 * evolvewp-predictiveerp Development Tabs JavaScript
 *
...
(function($) {
    ...
    function initdevelopment() {
        console.log('evolvewp-predictiveerp Development Tabs JS loaded');
    ...
    function initAccordions() {
        ...
        if ($('.evolvewp-predictiveerp-accordion-container').length > 0) {
            ...
            $('.collapse-all').off('click').on('click', function() {
                $('.evolvewp-predictiveerp-accordion-content').slideUp();
                $('.evolvewp-predictiveerp-accordion-icon')
    ...
    function initUILibrary() {
        ...
        if (typeof evolvewp-predictiveerpUILibrary !== 'undefined') {
            // UI Library JavaScript is loaded
            console.log('evolvewp-predictiveerpUILibrary object detected');
        }
    }
...
jQuery(document).ready(function($) {
    ...
    $('.refresh-github-data').on('click', function(e) {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'evolvewp_erp_refresh_github_data',
                nonce: evolvewp_erp_dev_tabs.nonce
            },
    ...
    $('.section-toggle-checkbox').on('change', function() {
        ...
        localStorage.setItem('evolvewp_erp_ui_library_sections', JSON.stringify(sectionStates));
    });
    ...
    var savedStates = localStorage.getItem('evolvewp_erp_ui_library_sections');
    if (savedStates) {
        try {
    ...
    $('.move-to-assets').on('click', function() {
        ...
        if (confirm(evolvewp-predictiveerpData.confirmMoveFile || 'Are you sure you want to move this file to the assets folder?')) {
            $btn.prop('disabled', true).text(evolvewp-predictiveerpData.movingText || 'Moving...');
            
            setTimeout(function() {
                alert(evolvewp-predictiveerpData.moveFileMessage || 'File move functionality would be implemented here');
                $btn.prop('disabled', false).text(evolvewp-predictiveerpData.moveToAssetsText || 'Move to Assets');
            }, 1000);
        }
    });
    ...
    $('.delete-file').on('click', function() {
        ...
        if (confirm(evolvewp-predictiveerpData.confirmDeleteFile || 'Are you sure you want to delete this file? This action cannot be undone.')) {
            $btn.prop('disabled', true).text(evolvewp-predictiveerpData.deletingText || 'Deleting...');
            
            setTimeout(function() {
                alert(evolvewp-predictiveerpData.deleteFileMessage || 'File delete functionality would be implemented here');
                $btn.prop('disabled', false).text(evolvewp-predictiveerpData.deleteText || 'Delete');
            }, 1000);
        }
    });
    ...
    if ($('.feature-status-tab').length > 0) {
        ...
        // Initialize evolvewp-predictiveerp-accordion functionality for feature status
        $('.evolvewp-predictiveerp-accordion-header').on('click', function() {
            var $content = $(this).next('.evolvewp-predictiveerp-accordion-content');
            $content.slideToggle();
            $(this).find('.evolvewp-predictiveerp-accordion-icon')
    ...
    $('#load-log-file').on('click', function() {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'evolvewp_erp_load_log_file',
                log_file: logFile,
                security: evolvewp_erp_dev_tabs.nonce
            },
            success: function(response) {
                if (response.success) {
    ...
    // Tasks functionality
    if (typeof evolvewp-predictiveerpTasksData !== 'undefined') {
        $('.start-task').on('click', function(e) {
            e.preventDefault();
            var taskId = $(this).data('task-id');
            var taskSource = $(this).data('task-source');
            
            localStorage.setItem('evolvewp_erp_current_task_id', taskId);
            localStorage.setItem('evolvewp_erp_current_task_source', taskSource);
            
            $(this).closest('tr').attr('data-status', 'in-progress');
            
            if (taskSource !== 'github') {
                var taskData = null;
                for (var i = 0; i < evolvewp-predictiveerpTasksData.tasks.length; i++) {
                    if (evolvewp-predictiveerpTasksData.tasks[i].id === taskId) {
                        taskData = evolvewp-predictiveerpTasksData.tasks[i];
                        break;
                    }
                }
                ...
                if (taskData) {
                    ...
                    $.ajax({
                        url: evolvewp_erp_dev_tabs.ajaxurl,
                        type: 'POST',
                        data: {
                            action: 'evolvewp_erp_create_github_issue',
                            nonce: evolvewp_erp_dev_tabs.nonce,
                        ...
                        success: function(response) {
                            if (response.success) {
                                localStorage.setItem('evolvewp_erp_GITHUB_issue_number', response.data.number);
                                localStorage.setItem('evolvewp_erp_GITHUB_issue_url', response.data.html_url);
                                window.location.href = 'admin.php?page=evolvewp_erp_development&tab=current_task';
                            } else {
