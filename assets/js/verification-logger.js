/**
 * EvolveWP PredictiveERP Verification JavaScript Logger
...
window.EvolveWP PredictiveERPVerificationLogger = {
    ...
    init: function() {
        ...
        this.stepCounts = {};
        console.log('EvolveWP_ERP_Verification: Logging initialized');
    },
    ...
    logStep: function(stepName, inputCount, outputCount, details = {}) {
        ...
        if (dataLoss) {
            console.error(`EvolveWP_ERP_Verification: DATA LOSS in ${stepName}: ${inputCount} → ${outputCount} (lost ${lossAmount})`, logData);
        } else {
            console.log(`EvolveWP_ERP_Verification: ${stepName}: ${inputCount} → ${outputCount} (+${elapsed}ms)`, logData);
        }
        
        // Use unified logger if available
        if (window.EvolveWP PredictiveERPLogger) {
            window.EvolveWP PredictiveERPLogger.trace('VERIFICATION_STEP', `${stepName}: ${inputCount} → ${outputCount}`, logData);
        }
    },
    ...
    logFileProcessing: function(loopId, fileName, resultCount, details = {}) {
        ...
        if (!this.stepCounts[loopKey]) {
            ...
            console.log(`EvolveWP_ERP_Verification: Starting file loop: ${loopId}`);
        }
        ...
        if (this.stepCounts[loopKey] % 10 === 0 || details.forceLog) {
            console.log(`EvolveWP_ERP_Verification: File ${this.stepCounts[loopKey]}: ${fileName} → ${resultCount} results`, {
                loopId: loopId,
                fileName: fileName,
        ...
        // Use unified logger
        if (window.EvolveWP PredictiveERPLogger) {
            window.EvolveWP PredictiveERPLogger.loopTrace(loopId, {
                fileName: fileName,
                resultCount: resultCount,
                ...details
            });
    ...
    logAjaxPreparation: function(operation, dataCount, payloadSize = null) {
        ...
        console.log(`EvolveWP_ERP_Verification: AJAX_PREP - ${operation}: ${dataCount} items` + 
                   (payloadSize ? ` (${payloadSize} bytes)` : ''), details);
        
        // Use unified logger
        if (window.EvolveWP PredictiveERPLogger) {
            window.EvolveWP PredictiveERPLogger.trace('AJAX_PREP', `${operation}: ${dataCount} items`, details);
        }
    },
    ...
    logHashFilter: function(totalFiles, changedFiles, skippedFiles) {
        ...
        console.log(`EvolveWP_ERP_Verification: HASH_FILTER - ${totalFiles} files, ${changedFiles} changed, ${skippedFiles} skipped`, hashData);
    ...
    logTransmission: function(operation, originalCount, transmittedCount, serializedSize = null) {
        ...
        if (serializedSize) {
            console.log(`EvolveWP_ERP_Verification: TRANSMISSION - ${operation}: ${originalCount} → ${transmittedCount} (${serializedSize} bytes)`, transmissionData);
        } else {
            console.log(`EvolveWP_ERP_Verification: TRANSMISSION - ${operation}: ${originalCount} → ${transmittedCount}`, transmissionData);
        }
    ...
    getSummary: function() {
        ...
        console.log('EvolveWP_ERP_Verification: SUMMARY', summary);
        return summary;
    },
     ...
     */
        ...
        console.log(`EvolveWP_ERP_Verification: Completed in ${summary.duration} with ${summary.totalSteps} steps`);
        
        // Use unified logger
        if (window.EvolveWP PredictiveERPLogger) {
            window.EvolveWP PredictiveERPLogger.endContext(summary);
        }
    }
};
...
if (typeof wpVerificationContext !== 'undefined' && wpVerificationContext) {
    EvolveWP PredictiveERPVerificationLogger.init();
}