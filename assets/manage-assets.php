<?php
/**
 * EvolveWP PredictiveERP Asset Management
 * 
...
class EvolveWP_ERP_Asset_Manager {
    ...
    private function log_missing_asset($type, $name, $path) {
        ...
        if (get_option('evolvewp_erp_log_missing_assets', 'yes') === 'yes') {
            EvolveWP_ERP_Logging_Helper::log_error(
                'missing_asset',
...
if (!isset($evolvewp_erp_assets)) {
    $evolvewp_erp_assets = new EvolveWP_ERP_Asset_Manager();
}

function evolvewp_erp_get_asset($type, $name) {
    global $evolvewp_erp_assets;
    return $evolvewp_erp_assets->get_asset_path($type, $name);
}

function evolvewp_erp_get_asset_url($type, $name) {
    global $evolvewp_erp_assets;
    return $evolvewp_erp_assets->get_asset_url($type, $name);
}
