<?php
/**
 * EvolveWP PredictiveERP Asset Queue System
 * 
 * Centralized asset enqueueing based on page detection
 * 
 * @package EvolveWP PredictiveERP/Assets
 * @version 1.2.0
 */
...
class EvolveWP_ERP_Asset_Queue {
    ...
    public function __construct() {
        if (!class_exists('EvolveWP_ERP_Asset_Manager')) {
            require_once EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'assets/manage-assets.php';
        }
        
        global $evolvewp_erp_assets;
        $this->asset_manager = $evolvewp_erp_assets;
    ...
    private function enqueue_style($name, $asset) {
        ...
        $deps = array_map( function( $dep ) {
            ...
            if ( 0 === strpos( $dep, 'evolvewp-predictiveerp-' ) || 0 === strpos( $dep, 'wp-' ) ) {
                return $dep;
            }
            return 'evolvewp-predictiveerp-' . $dep;
        }, $asset['dependencies'] );
        
        wp_enqueue_style(
            'evolvewp-predictiveerp-' . $name,
            $url,
            $deps,
    ...
    private function enqueue_script($name, $asset) {
        ...
        // Prefix dependency handles to match registered handle format.
        $deps = array_map( function( $dep ) {
            if ( 0 === strpos( $dep, 'evolvewp-predictiveerp-' ) || 0 === strpos( $dep, 'wp-' ) || 'jquery' === $dep ) {
                return $dep;
            }
            return 'evolvewp-predictiveerp-' . $dep;
        }, $asset['dependencies'] );
    ...
    public function missing_assets_notice() {
        ...
        <div class="notice notice-warning is-dismissible">
            <p>
                <strong>EvolveWP PredictiveERP:</strong>
                <?php
                /* translators: %d: Number of missing asset files */
                echo esc_html(sprintf(_n('%d asset file is missing', '%d asset files are missing', $count, 'evolvewp-predictiveerp'), $count));
                ?>
            </p>
...
if (!class_exists('EvolveWP_ERP_Asset_Queue_Instance')) {
    class EvolveWP_ERP_Asset_Queue_Instance {
        private static $instance = null;
        
        public static function get_instance() {
            if (self::$instance === null) {
                self::$instance = new EvolveWP_ERP_Asset_Queue();
            }
            return self::$instance;
        }
    }
    
    EvolveWP_ERP_Asset_Queue_Instance::get_instance();
}
