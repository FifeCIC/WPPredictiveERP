...
```php
$scheduler = EvolveWP_ERP_Task_Scheduler::instance();
...
- `$group` (string) - Group name for organization (default: 'evolvewp-predictiveerp')
...
- `$group` (string) - Group name (default: 'evolvewp-predictiveerp')
...
add_action('daily_cleanup', function() {
    // Delete old logs
    global $wpdb;
    $wpdb->query("DELETE FROM {$wpdb->prefix}evolvewp_erp_logs WHERE created_at < DATE_SUB(NOW(), INTERVAL 30 DAY)");
});
```
...
## Migration from EvolveWP_ERP_Background_Process

### Old Way

```php
class My_Process extends EvolveWP_ERP_Background_Process {
...
// ✅ Good - specific name
$scheduler->schedule_single('evolvewp_erp_process_user_data', $args);
```
