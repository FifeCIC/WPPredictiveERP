...
3. **Override hook** — the `evolvewp_erp_user_can` filter lets EvolveWP PredictiveERP intercept
...
if ( evolvewp_erp_user_can( 'evolvewp_erp_manage_settings' ) ) {
    // show settings page
}

// Check for a specific user.
if ( evolvewp_erp_user_can( 'evolvewp_erp_view_logs', $user_id ) ) {
...
| `evolvewp_erp_manage_settings` | core | administrator | Access the settings page |
| `evolvewp_erp_view_development` | development | administrator | Access development tools |
| `evolvewp_erp_manage_connectors` | api | administrator | Configure API credentials |
| `evolvewp_erp_execute_connectors` | api | administrator | Run connector actions |
| `evolvewp_erp_view_logs` | development | administrator | View logs and debug output |

When cloning WPSeed to a new plugin, these are renamed with the plugin's prefix
(e.g. `evolvewp_erp_manage_settings`).
...
// Returns: array( 'evolvewp_erp_manage_settings' => array( 'label' => ..., ... ), ... )
```
...
```php
$cap = Capability_Manager::get( 'evolvewp_erp_manage_settings' );
...
add_filter( 'evolvewp_erp_user_can', function( $result, $capability, $user_id ) {
    if ( 42 === $user_id ) {
...
This filter is how EvolveWP PredictiveERP will implement:
...
2. The global accessor becomes `evolvewp_erp_user_can()`.
3. All default capability names are renamed (e.g. `evolvewp_erp_manage_settings`).
