...
add_action('evolvewp_erp_carbon_fields_register', 'my_theme_options');
function my_theme_options() {
    Container::make('theme_options', __('My Settings'))
        ->set_page_parent('evolvewp-predictiveerp-settings')
        ->add_fields(array(
...
```php
add_action('evolvewp_erp_carbon_fields_register', 'my_post_meta');
...
```php
// Create options page
EvolveWP_ERP_Carbon_Fields::create_options_page('My Settings', 'my-settings', 'evolvewp-predictiveerp-settings');

// Create post meta
EvolveWP_ERP_Carbon_Fields::create_post_meta('Product Info', array('product'));

// Create term meta
EvolveWP_ERP_Carbon_Fields::create_term_meta('Category Fields', array('category'));

// Create user meta
EvolveWP_ERP_Carbon_Fields::create_user_meta('User Profile');
```
...
1. **Use Prefixes**: Always prefix field names (`evolvewp_erp_field_name`)
...
- GitHub: https://github.com/ryanbayne/evolvewp-predictiveerp/issues
