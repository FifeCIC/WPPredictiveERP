...
│                    Your Plugin Code                      │
│                                                          │
│   $github = evolvewp_erp_connector( 'github' );               │
│   $result = $github->execute( 'create_issue', $params ); │
└──────────────┬───────────────────────────────────────────┘
               │
               ▼
┌──────────────────────┐     ┌──────────────────────────┐
│   EvolveWP_ERP_API_Factory │────▶│  EvolveWP_ERP_API_Directory    │
│                      │     │                          │
...
│   - API call logging via EvolveWP_ERP_API_Logging              │
...
add_action( 'init', function() {
    EvolveWP_ERP_API_Directory::register( 'github', array(
        'name'        => 'GitHub',
...
$issues = $github->execute( 'get_issues', array(
    'repo'  => 'evolvewp/evolvewp-predictiveerp',
    'state' => 'open',
) );
...
```
evolvewp_erp_api_{provider_id}_key       → API key / access token
evolvewp_erp_api_{provider_id}_secret    → API secret (if needed)
evolvewp_erp_api_{provider_id}_url       → Base URL (if configurable)
```
...
```
evolvewp_erp_api_{provider_id}_{account_id}_key
evolvewp_erp_api_{provider_id}_{account_id}_secret
evolvewp_erp_api_{provider_id}_{account_id}_url
```
...
// Named account.
$github_org = evolvewp_erp_connector( 'github', 'my-org' );
```

### Injecting Credentials from Other Sources

Use the `evolvewp_erp_connector_credentials` filter to load credentials from
environment variables, a secrets manager, or any other source:

```php
add_filter( 'evolvewp_erp_connector_credentials', function( $args, $provider_id, $account_id ) {
...
$caps = EvolveWP_ERP_API_Directory::get_provider_capabilities( 'github' );
...
### evolvewp_erp_api_providers

Filter the complete list of registered connector providers.

```php
add_filter( 'evolvewp_erp_api_providers', function( $providers ) {
    // Remove a provider.
...
### evolvewp_erp_connector_credentials
...
### evolvewp_erp_connector_request_args

Filter the `wp_remote_request()` arguments before every API call.

```php
add_filter( 'evolvewp_erp_connector_request_args', function( $args, $url, $provider_id, $endpoint ) {
    if ( 'github' === $provider_id ) {
...
| `functions.php` | `evolvewp_erp_connector()` global accessor |
...
The `evolvewp_erp_connector()` function becomes `yourprefix_connector()` and
all option names change from `evolvewp_erp_api_*` to `yourprefix_api_*`.
