...
add_action('evolvewp_erp_ecosystem_register', function() {
    evolvewp_erp_ecosystem()->register_plugin('myplugin', array(
        'name' => 'My Plugin',
        'version' => '1.0.0',
...
evolvewp_erp_ecosystem()->register_shared_resource('logging', function($plugin_slug) {
...
evolvewp_erp_ecosystem()->register_shared_resource('background_tasks', function() {
...
```php
if (evolvewp_erp_ecosystem()->is_ecosystem_mode()) {
    // 2+ plugins installed
    // Use shared menus
} else {
    // Single plugin
...
```php
// In your plugin
evolvewp_erp_ecosystem()->register_shared_resource('logging', 'myplugin_render_logs', 10);
...
add_filter('evolvewp_erp_ecosystem_available_plugins', function($plugins) {
    $plugins['myplugin'] = array(
        'name' => 'My Plugin',
        'description' => 'Does amazing things',
        'download_url' => 'https://github.com/user/myplugin/archive/main.zip',
        'required_by' => array(),
        'integrates_with' => array('evolvewp-predictiveerp', 'evolvewp-predictiveerp'),
    );
    return $plugins;
});
```
...
if (evolvewp_erp_ecosystem()->is_registered('evolvewp-predictiveerp')) {
    // wpseed is installed
    // Enable evolvewp-predictiveerp-specific features
}

// Get all installed plugins
$plugins = evolvewp_erp_ecosystem()->get_plugins();
...
```php
// In your plugin
add_action('evolvewp_erp_ecosystem_plugin_registered', function($slug, $plugin) {
    if ($slug === 'evolvewp-predictiveerp') {
        // wpseed was just registered
        // Enable wpseed integration features
        update_option('myplugin_evolvewp_erp_integration', true);
    }
}, 10, 2);
```
...
```php
add_action('evolvewp_erp_ecosystem_register', 'myplugin_register_ecosystem');
```
...
add_action('admin_menu', function() {
    if (evolvewp_erp_ecosystem()->use_shared_menu()) {
...
add_action('evolvewp_erp_ecosystem_plugin_registered', function($slug, $plugin) {
    if ($slug === 'evolvewp-predictiveerp') {
        ...
        update_option('myplugin_evolvewp_erp_enabled', true);
    }
}, 10, 2);
```
...
**`evolvewp_erp_ecosystem()`**
- Returns: `EvolveWP_ERP_Ecosystem_Registry` instance
...
```php
// Debug
var_dump(evolvewp_erp_ecosystem()->is_ecosystem_mode());
var_dump(evolvewp_erp_ecosystem()->get_plugins());
var_dump(get_option('evolvewp_erp_ecosystem_menu_location'));
```
...
```php
// Debug
var_dump(evolvewp_erp_ecosystem()->get_shared_resources('logging'));
```
...
- GitHub: [Report Issue](https://github.com/ryanbayne/evolvewp-predictiveerp/issues)
- Documentation: [Wiki](https://github.com/ryanbayne/evolvewp-predictiveerp/wiki)
