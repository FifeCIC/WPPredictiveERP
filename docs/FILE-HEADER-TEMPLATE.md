 ...
 * DATA FLOW:
 *   Input  → none
 *   Output → defines EVOLVEWP_ERP_* constants, requires vendor/autoload.php and loader.php
 *
 * @package  WPSeed
 * @since    1.0.0
 */
 ...
 *
 * DEPENDS ON:
 *   - EvolveWP_ERP_Admin_Settings in includes/admin/admin-settings.php
 *   - global evolvewp_erp_get_screen_ids() in includes/admin/admin-functions.php
 *
 * CONSUMED BY:
 *   - Hook: admin_menu (registered in includes/admin/admin-menus.php)
 *
 * DATA FLOW:
 *   Input  → get_option( 'evolvewp_erp_settings' )
 ...
 *
 * CONSUMED BY:
 *   - Hook: wp_ajax_evolvewp_erp_save_settings
 *
 * DATA FLOW:
...
<?php
/**
 * Plugin settings storage — reads and writes the evolvewp_erp_settings option.
 *
 * ROLE: data-model
 *
 ...
 *
 * CONSUMED BY:
 *   - EvolveWP_ERP_Admin_Settings::save() in includes/admin/admin-settings.php
 *   - EvolveWP_ERP_Admin_Settings::get() in includes/admin/admin-settings.php
 *
 * DATA FLOW:
 *   Input  → get_option( 'evolvewp_erp_settings' )
 *   Output → update_option( 'evolvewp_erp_settings', $data )
 *
 * @package  WPSeed
 * @category Core
 * @since    1.0.0
 ...
 *
 * DEPENDS ON:
 *   - $data array prepared by EvolveWP_ERP_Admin_Dashboard::render()
 *
 * CONSUMED BY:
 *   - EvolveWP_ERP_Admin_Dashboard::render() in includes/Admin/Dashboard.php
 *
 * DATA FLOW:
 ...
 * CONSUMED BY:
 *   - Hook: rest_api_init
 *   - Route: GET/POST /wp-json/evolvewp-predictiveerp/v1/settings
 *
 * DATA FLOW:
 ...
 *   - Hook: evolvewp_erp_ecosystem_register
 *
 * DATA FLOW:
