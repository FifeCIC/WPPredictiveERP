...
1. Download the latest release from [GitHub](https://github.com/ryanbayne/evolvewp-predictiveerp/releases)
2. Extract to `wp-content/plugins/`
3. Rename folder from `evolvewp-predictiveerp-main` to `wpseed`

### Method 3: WP-CLI
```bash
wp plugin install https://github.com/ryanbayne/evolvewp-predictiveerp/archive/main.zip --activate
```
...
- **Command Line**: 
  ```bash
  # Linux/Mac
  find . -type f -name "*.php" -exec sed -i 's/evolvewp-predictiveerp/myawesomeplugin/g' {} +
  
  # Windows (PowerShell)
  Get-ChildItem -Recurse -Filter *.php | ForEach-Object { (Get-Content $_.FullName) -replace 'evolvewp-predictiveerp', 'myawesomeplugin' | Set-Content $_.FullName }
  ```
...
```php
<?php
class MyAwesomePlugin_REST_Books extends EvolveWP_ERP_REST_Controller {
...
class MyAwesomePlugin_Settings_Books extends EvolveWP_ERP_Settings_Page {
...
2. Check `wp_options` for `evolvewp_erp_db_version`
3. Manually run: `EvolveWP_ERP_Install::install()`
...
- [GitHub Discussions](https://github.com/ryanbayne/evolvewp-predictiveerp/discussions)
- [Documentation](https://github.com/ryanbayne/evolvewp-predictiveerp/wiki)
...
### Key Classes
- `EvolveWP_ERP_Install` - Installation and setup
- `EvolveWP_ERP_REST_Controller` - REST API base
- `EvolveWP_ERP_Logger` - Logging system
- `EvolveWP_ERP_AI_Assistant` - AI integration
- `EvolveWP_ERP_Settings_Page` - Settings framework

### Key Functions
- `evolvewp_erp_log()` - Log messages
- `evolvewp_erp_get_option()` - Get plugin option
- `evolvewp_erp_help_tip()` - Add tooltip
- `evolvewp_erp_clean()` - Sanitize data

### Hooks
- `evolvewp_erp_loaded` - Plugin loaded
- `evolvewp_erp_init` - Plugin initialized
- `evolvewp_erp_settings_save_{tab}` - Settings saved
- `evolvewp_erp_admin_mainviews` - Add admin views
