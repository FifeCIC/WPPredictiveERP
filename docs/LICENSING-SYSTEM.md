...
$license_manager = new EvolveWP_ERP_License_Manager(
...
$extension_installer = new EvolveWP_ERP_Extension_Installer(
...
```php
define( 'EVOLVEWP_ERP_LICENSE_SERVER', 'https://your-server.com' );
define( 'EVOLVEWP_ERP_PRODUCT_ID', 'evolvewp-predictiveerp-plugin' );
```
...
### Activate License
```
POST /wp-json/evolvewp-predictiveerp-license/v1/activate
```
...
### Deactivate License
```
POST /wp-json/evolvewp-predictiveerp-license/v1/deactivate
```

### Check License Status
```
POST /wp-json/evolvewp-predictiveerp-license/v1/check
```
...
### Check for Updates
```
POST /wp-json/evolvewp-predictiveerp-license/v1/update_check
```
...
### Get Plugin Info
```
POST /wp-json/evolvewp-predictiveerp-license/v1/plugin_info
```

### Get License Info
```
POST /wp-json/evolvewp-predictiveerp-license/v1/info
```
...
### List Extensions
```
GET /wp-json/evolvewp-predictiveerp-extensions/v1/list
```
...
### Download Extension
```
POST /wp-json/evolvewp-predictiveerp-extensions/v1/download
```
...
```php
$license_manager = new EvolveWP_ERP_License_Manager(
    EVOLVEWP_ERP_LICENSE_SERVER,
    EVOLVEWP_ERP_PRODUCT_ID,
    EVOLVEWP_ERP_VERSION
);
...
```php
$installer = new EvolveWP_ERP_Extension_Installer( EVOLVEWP_ERP_LICENSE_SERVER );
...
```php
check_admin_referer( 'evolvewp_erp_license_action' );
```
...
```php
add_filter( 'evolvewp_erp_license_validation', function( $is_valid, $license_key ) {
...
```php
add_filter( 'evolvewp_erp_update_check_interval', function( $interval ) {
...
```php
add_filter( 'evolvewp_erp_extensions_list', function( $extensions ) {
...
2. Check update check transient: `evolvewp_erp_license_check_{product_id}`
