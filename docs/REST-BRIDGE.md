...
│       'capability' => 'evolvewp_erp_manage_settings',              │
│       'label'      => 'List Projects',                       │
│   ) );                                                       │
└──────────────┬──────────────────────────────────────────────┘
               │
...
│   Permission factory ───────── evolvewp_erp_user_can() checks      │
...
```bash
# Test connection
curl -X GET https://example.com/wp-json/evolvewp-predictiveerp/v1/connector/github/test \
  -H "X-WP-Nonce: {nonce}"

# List capabilities
curl -X GET https://example.com/wp-json/evolvewp-predictiveerp/v1/connector/github/capabilities \
  -H "X-WP-Nonce: {nonce}"

# Execute an action
curl -X POST https://example.com/wp-json/evolvewp-predictiveerp/v1/connector/github/create_issue \
  -H "X-WP-Nonce: {nonce}" \
  -H "Content-Type: application/json" \
  -d '{"repo": "evolvewp/evolvewp-predictiveerp", "title": "Bug fix", "body": "Details..."}'
```
...
```
Q → curl → /wp-json/evolvewp-predictiveerp/v1/connector/github/create_issue
...
```php
$all = evolvewp_erp_rest_endpoints();

// Filter by source
$manual    = evolvewp_erp_rest_endpoints( 'manual' );
$connector = evolvewp_erp_rest_endpoints( 'connector' );
```
...
- EvolveWP PredictiveERP Feature Gate (knows what's available)
...
### evolvewp_erp_rest_bridge_before_register
...
```php
add_action( 'evolvewp_erp_rest_bridge_before_register', function() {
    REST_Bridge::register( 'wpseed/v1', '/late-endpoint', array( ... ) );
} );
```

### evolvewp_erp_rest_bridge_registered

Fires after all routes are registered. Receives the full endpoint metadata array.

```php
add_action( 'evolvewp_erp_rest_bridge_registered', function( $endpoints ) {
...
| `functions.php` | `evolvewp_erp_rest_endpoints()` global accessor |
...
2. The namespace changes (e.g. `wpseed/v1` → `evolvewp-predictiveerp/v1`).
3. The `evolvewp_erp_rest_endpoints()` function becomes `yourprefix_rest_endpoints()`.
