...
>
> All `wpseed` / `EVOLVEWP_ERP_` / `EvolveWP_ERP_` strings are placeholders. When a new
...
- **7.2** ✅ Replaced all `--tp-` → `--evolvewp-predictiveerp-` (46 CSS), `.tp-` → `.evolvewp-predictiveerp-` (6 CSS, 1 JS, 7 PHP)
...
- **7.5** ✅ Removed invalid `@extend` from 27 files. Removed duplicate button defs from forms.css. Removed TradePress sections from forms.css. Scoped `reset.css` and `typography.css` to `.evolvewp-predictiveerp-admin-wrap`
...
- **Capability Manager** (`Core/Capability_Manager.php`) — Registration with metadata, install/uninstall lifecycle, `evolvewp_erp_user_can()` with filter hook. 7 default caps. Install.php delegates to it. Docs: `docs/CAPABILITIES.md`
...
| `evolvewp_erp_ecosystem()` | Ecosystem Registry singleton | 3.0.0 |
| `evolvewp_erp_log()` | Logger singleton | 3.0.0 |
| `evolvewp_erp_trace()` | Records a trace entry | 3.0.0 |
| `evolvewp_erp_connector()` | API connector instance | 3.1.0 |
| `evolvewp_erp_user_can()` | Capability check (bool) | 3.1.0 |
| `evolvewp_erp_rest_endpoints()` | Registered REST endpoint metadata | 3.1.0 |

---

## Items Deferred to EvolveWP PredictiveERP

| Item | Why deferred | Where planned |
|---|---|---|
| Typed Settings Manager | Runtime feature, not boilerplate | EvolveWP PredictiveERP Stage B |
| Database Abstraction (Table + Query) | Needed by plugins with custom tables | EvolveWP PredictiveERP Stage B |
| Frontend Template System | Needed by plugins with public UI | EvolveWP PredictiveERP Stage D |
| Frontend REST API | Public-facing endpoints for portals | EvolveWP PredictiveERP Stage D |
| Cloud Services Foundation | Backup, storage, hack detection | EvolveWP PredictiveERP Stage D |
| Webhook System | Event notifications to external services | EvolveWP PredictiveERP Stage D |
...
| Unmigrated class references | `EvolveWP_ERP_Developer_Mode`, `EvolveWP_ERP_Notifications`, `EvolveWP_ERP_API_Logging` |
