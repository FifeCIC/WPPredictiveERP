...
as_schedule_single_action( time() + 3600, 'evolvewp_erp_process_data', array( 'user_id' => 123 ) );

// Schedule a recurring action (every hour)
as_schedule_recurring_action( time(), HOUR_IN_SECONDS, 'evolvewp_erp_hourly_sync' );

// Hook your callback
add_action( 'evolvewp_erp_process_data', 'evolvewp_erp_handle_data_processing' );
function evolvewp_erp_handle_data_processing( $user_id ) {
    // Your processing logic here
    error_log( 'Processing data for user: ' . $user_id );
}

// Check if action is scheduled
if ( as_next_scheduled_action( 'evolvewp_erp_process_data' ) ) {
    // Action is already scheduled
}

// Cancel scheduled action
as_unschedule_action( 'evolvewp_erp_process_data', array( 'user_id' => 123 ) );
```
...
add_action( 'after_setup_theme', 'evolvewp_erp_load_carbon_fields' );
function evolvewp_erp_load_carbon_fields() {
    \Carbon_Fields\Carbon_Fields::boot();
}

// Register fields
add_action( 'carbon_fields_register_fields', 'evolvewp_erp_register_custom_fields' );
function evolvewp_erp_register_custom_fields() {
    
    // Theme Options Page
    Container::make( 'theme_options', __( 'Theme Settings' ) )
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_erp_site_tagline', 'Site Tagline' ),
            Field::make( 'textarea', 'evolvewp_erp_footer_text', 'Footer Text' ),
            Field::make( 'image', 'evolvewp_erp_logo', 'Logo' ),
        ) );
    ...
    Container::make( 'post_meta', __( 'Custom Fields' ) )
        ...
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_erp_subtitle', 'Subtitle' ),
            Field::make( 'checkbox', 'evolvewp_erp_featured', 'Featured Post' ),
            Field::make( 'complex', 'evolvewp_erp_gallery', 'Gallery' )
                ->add_fields( array(
    ...
    Container::make( 'user_meta', __( 'Social Profiles' ) )
        ->add_fields( array(
            Field::make( 'text', 'evolvewp_erp_twitter', 'Twitter URL' ),
            Field::make( 'text', 'evolvewp_erp_linkedin', 'LinkedIn URL' ),
        ) );
}

// Retrieve values
$tagline = carbon_get_theme_option( 'evolvewp_erp_site_tagline' );
$subtitle = carbon_get_post_meta( get_the_ID(), 'evolvewp_erp_subtitle' );
$twitter = carbon_get_user_meta( $user_id, 'evolvewp_erp_twitter' );
```
...
```php
require_once EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'includes/libraries/parsedown/Parsedown.php';
...
class EvolveWP_ERP_Custom_Field extends \Carbon_Fields\Field\Field {
...
### From EvolveWP_ERP_Background_Process to Action Scheduler

**Old Code:**
```php
class My_Background_Process extends EvolveWP_ERP_Background_Process {
...
```php
// Schedule action
as_enqueue_async_action( 'evolvewp_erp_my_process', array( 'id' => 1 ) );

// Hook callback
add_action( 'evolvewp_erp_my_process', 'evolvewp_erp_process_item' );
function evolvewp_erp_process_item( $id ) {
    // Process item
}
```
...
**Old Code:**
```php
add_settings_section( 'evolvewp_erp_section', 'Settings', null, 'evolvewp-predictiveerp' );
add_settings_field( 'evolvewp_erp_api_key', 'API Key', 'evolvewp_erp_api_key_callback', 'evolvewp-predictiveerp', 'evolvewp_erp_section' );
register_setting( 'evolvewp-predictiveerp', 'evolvewp_erp_api_key' );
```
...
Container::make( 'theme_options', 'Settings' )
    ->add_fields( array(
        Field::make( 'text', 'evolvewp_erp_api_key', 'API Key' ),
    ) );
```
