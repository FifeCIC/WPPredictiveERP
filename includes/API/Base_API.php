 ...
 *
 * DEPENDS ON:
 *   - EvolveWP PredictiveERP\API\Connector_Interface (implements)
 ...
 *   - EvolveWP_ERP_API_Logging (global class, not yet namespaced)
 *
 * CONSUMED BY:
 ...
 *   Output → Parsed JSON response data, logged API calls via EvolveWP_ERP_API_Logging
 *
...
namespace EvolveWP\PredictiveERP\API;
...
abstract class Base_API implements Connector_Interface {
	...
	public function execute( $action, $params = array() ) {
		...
		if ( ! isset( $capabilities[ $action ] ) ) {
			return new \WP_Error(
				...
				sprintf(
					...
					__( 'Action "%1$s" is not supported by the %2$s connector.', 'evolvewp-predictiveerp' ),
					$action,
					$this->provider_id
				),
				array( 'status' => 400 )
		...
		return new \WP_Error(
			...
			sprintf(
				...
				__( 'Action "%1$s" is declared but not implemented. Override execute() in %2$s.', 'evolvewp-predictiveerp' ),
				$action,
				static::class
			),
			array( 'status' => 501 )
	...
	protected function make_request( $endpoint, $params = array(), $method = 'GET' ) {
		...
		if ( class_exists( '\\EvolveWP_ERP_API_Logging', false ) ) {
			$call_id = \EvolveWP_ERP_API_Logging::log_call(
				$this->provider_id,
				$endpoint,
				$method,
				'pending'
		 ...
		 */
		$args = apply_filters( 'evolvewp_erp_connector_request_args', $args, $url, $this->provider_id, $endpoint );

		$response = wp_remote_request( $url, $args );

		if ( is_wp_error( $response ) ) {
			if ( $call_id && class_exists( '\\EvolveWP_ERP_API_Logging', false ) ) {
				\EvolveWP_ERP_API_Logging::log_error( $call_id, 'request_failed', $response->get_error_message() );
				\EvolveWP_ERP_API_Logging::update_call_outcome( $call_id, 'Error: ' . $response->get_error_message(), 'error' );
			}
			return $response;
		}
		...
		if ( $status_code >= 400 ) {
			...
			if ( $call_id && class_exists( '\\EvolveWP_ERP_API_Logging', false ) ) {
				\EvolveWP_ERP_API_Logging::log_error( $call_id, 'http_' . $status_code, $error_message );
				\EvolveWP_ERP_API_Logging::update_call_outcome( $call_id, 'HTTP ' . $status_code . ': ' . $error_message, 'error' );
			}
		...
		if ( $call_id && class_exists( '\\EvolveWP_ERP_API_Logging', false ) ) {
			\EvolveWP_ERP_API_Logging::update_call_outcome( $call_id, 'HTTP ' . $status_code, 'success' );
		}
	...
	protected function connection_success( $message = '', $data = array() ) {
		if ( empty( $message ) ) {
			$message = sprintf(
				...
				__( 'Successfully connected to %s.', 'evolvewp-predictiveerp' ),
				$this->provider_id
			);
		}
	...
	protected function connection_failure( $message = '', $data = array() ) {
		if ( empty( $message ) ) {
			$message = sprintf(
				/* translators: %s: provider ID */
				__( 'Failed to connect to %s.', 'evolvewp-predictiveerp' ),
				$this->provider_id
			);
		}
