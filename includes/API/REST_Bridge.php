 ...
 *   - EvolveWP PredictiveERP\Core\Capability_Manager (for evolvewp_erp_user_can checks)
 *   - EvolveWP_ERP_API_Factory (for connector route generation)
 *   - EvolveWP_ERP_API_Directory (for connector capability discovery)
 *
 * CONSUMED BY:
 *   - EvolveWP PredictiveERP\API\REST_Controller::register_endpoint() (convenience wrapper)
 *   - Concrete controller classes (register their endpoints here)
 *   - EvolveWP PredictiveERP Feature Gate (queries registered endpoints)
...
class REST_Bridge {
	 ...
	 * @since  3.1.0
	 *
	 * @param  string $namespace REST namespace (e.g. 'evolvewp-predictiveerp/v1').
	 ...
	 *     @type string   $capability  Capability required. Checked via evolvewp_erp_user_can().
	 ...
	 *                                      Default 'evolvewp_erp_execute_connectors'.
	 *     @type string $capability_manage  Capability for test/capabilities.
	 *                                      Default 'evolvewp_erp_manage_connectors'.
	 * }
	 * @return void
	 */
	public static function register_connector_routes( $namespace, $provider_id, $args = array() ) {
		$defaults = array(
			'capability_execute' => 'evolvewp_erp_execute_connectors',
			'capability_manage'  => 'evolvewp_erp_manage_connectors',
		);
	...
	public static function register_all_routes() {
		 ...
		 * @since 3.1.0
		 */
		do_action( 'evolvewp_erp_rest_bridge_before_register' );
		 ...
		 */
		do_action( 'evolvewp_erp_rest_bridge_registered', self::$endpoints );
	}
	...
	private static function make_permission_callback( $capability ) {
		...
		return static function () use ( $capability ) {
			return \evolvewp_erp_user_can( $capability );
		};
	}
	...
	private static function make_connector_callback( $provider_id, $internal_action ) {
		return static function ( \WP_REST_Request $request ) use ( $provider_id, $internal_action ) {
			$connector = \EvolveWP_ERP_API_Factory::create_from_settings( $provider_id );
