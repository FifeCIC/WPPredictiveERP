 ...
 *
 * CONSUMED BY:
 *   - functions.php: evolvewp_erp_user_can() global accessor
 *   - EvolveWP PredictiveERP\Core\Install: delegates capability installation here
 *   - REST controllers, admin pages, AJAX handlers — anywhere a permission
 *     check is needed
 *   - EvolveWP PredictiveERP (elevates this into cross-plugin permission management)
 *
 * DATA FLOW:
 ...
 *     Capability_Manager::register( 'evolvewp_erp_manage_settings', array(
 ...
 *
 *     // Check a capability.
 *     if ( Capability_Manager::user_can( 'evolvewp_erp_manage_settings' ) ) {
 *         // show settings page
 *     }
 *
...
class Capability_Manager {
	 ...
	 *                            plugin prefix (e.g. 'evolvewp_erp_manage_settings').
	 * @param  array  $args {
	...
	/**
	 * Register the default EvolveWP PredictiveERP capabilities.
	 *
	 * Called automatically on first use. These are the baseline caps that
	 * every EvolveWP PredictiveERP-based plugin starts with. Plugins add their own via
	...
	public static function register_defaults() {
		...
		self::register_many( array(
			'manage_evolvewp-predictiveerp' => array(
				'label'       => __( 'Manage EvolveWP PredictiveERP', 'evolvewp-predictiveerp' ),
				'description' => __( 'Full administrative access to the EvolveWP PredictiveERP plugin.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'core',
			),
			'code_evolvewp-predictiveerp' => array(
				'label'       => __( 'Code EvolveWP PredictiveERP', 'evolvewp-predictiveerp' ),
				'description' => __( 'Access developer-level features and code tools.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
			'evolvewp_erp_manage_settings' => array(
				'label'       => __( 'Manage Settings', 'evolvewp-predictiveerp' ),
				'description' => __( 'Access and modify the plugin settings page.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'core',
			),
			'evolvewp_erp_view_development' => array(
				'label'       => __( 'View Development Tools', 'evolvewp-predictiveerp' ),
				'description' => __( 'Access the Development page with architecture, roadmap, and debug tabs.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
			'evolvewp_erp_manage_connectors' => array(
				'label'       => __( 'Manage API Connectors', 'evolvewp-predictiveerp' ),
				'description' => __( 'Configure API credentials and test connector connections.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'api',
			),
			'evolvewp_erp_execute_connectors' => array(
				'label'       => __( 'Execute Connector Actions', 'evolvewp-predictiveerp' ),
				'description' => __( 'Run actions on external APIs via the connector system.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'api',
			),
			'evolvewp_erp_view_logs' => array(
				'label'       => __( 'View Logs', 'evolvewp-predictiveerp' ),
				'description' => __( 'Access API call logs, debug logs, and trace output.', 'evolvewp-predictiveerp' ),
				'grant_to'    => array( 'administrator' ),
				'group'       => 'development',
			),
		) );
	 ...
	 * allows EvolveWP PredictiveERP to intercept and extend permission logic later.
	 *
	 * @since  3.1.0
	 *
	...
	public static function user_can( $capability, $user_id = null ) {
		 ...
		 *
		 * Allows EvolveWP PredictiveERP or other plugins to override permission
		 ...
		 */
		$override = apply_filters( 'evolvewp_erp_user_can', null, $capability, $user_id );
	...
	public static function install() {
		 ...
		 */
		do_action( 'evolvewp_erp_capabilities_installed', self::$capabilities );
	}
	...
	public static function uninstall() {
		 ...
		 * @param array<string, array> $capabilities All registered capabilities.
		 */
		do_action( 'evolvewp_erp_capabilities_uninstalled', self::$capabilities );
	}
