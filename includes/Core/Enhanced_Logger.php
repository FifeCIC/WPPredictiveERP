 ...
 *
 * DEPENDS ON:
 *   - EvolveWP_ERP_Developer_Mode::is_dev_environment() in includes/classes/developer-mode.php
 *   - EvolveWP PredictiveERP\Core\Logger (optional — used for cross-referencing trace data)
 ...
 *   Output → {prefix}evolvewp_erp_debug_logs DB table
 *
...
class Enhanced_Logger {
	...
	public function save_logs() {
		...
		$cache_key = 'evolvewp_erp_logger_table_exists';
		$exists    = wp_cache_get( $cache_key, 'evolvewp_erp_logger' );

		if ( false === $exists ) {
			$exists = ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) === $table );
			wp_cache_set( $cache_key, $exists, 'evolvewp_erp_logger', 3600 );
		}
...
register_activation_hook( EVOLVEWP_ERP_PLUGIN_FILE, array( __NAMESPACE__ . '\\Enhanced_Logger', 'create_table' ) );
