 ...
 *   - EvolveWP_ERP_Admin_Notices (not yet migrated — referenced via global class name)
 *   - EvolveWP_ERP_Admin_Settings (not yet migrated — referenced via global class name)
 *   - global EvolveWP PredictiveERP() function in functions.php
 *
 * CONSUMED BY:
 ...
 *
 * DATA FLOW:
 *   Input  → evolvewp_erp_version option, evolvewp_erp_db_version option
...
class Install {
	...
	public static function install() {
		...
		// EvolveWP_ERP_Admin_Notices is not yet namespaced — load via legacy path.
		if ( ! class_exists( 'EvolveWP_ERP_Admin_Notices' ) ) {
			include_once EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'includes/admin/admin-notices.php';
		}
	...
	public static function plugin_row_meta( $links, $file ) {
		if ( $file === EVOLVEWP_ERP_PLUGIN_BASENAME ) {
			$row_meta = array(
				'docs'    => '<a href="' . esc_url( apply_filters( 'evolvewp_erp_docs_url', EVOLVEWP_ERP_DOCS ) ) . '">' . __( 'Docs', 'evolvewp-predictiveerp' ) . '</a>',
				'support' => '<a href="' . esc_url( apply_filters( 'evolvewp_erp_support_url', EVOLVEWP_ERP_GITHUB . '/issues' ) ) . '">' . __( 'Support', 'evolvewp-predictiveerp' ) . '</a>',
			);
	...
	private static function create_files() {
		...
		$files = array(
			array( 'base' => $upload_dir['basedir'] . '/evolvewp_erp_uploads', 'file' => 'index.html', 'content' => '' ),
			array( 'base' => EVOLVEWP_ERP_LOG_DIR, 'file' => '.htaccess', 'content' => 'deny from all' ),
			array( 'base' => EVOLVEWP_ERP_LOG_DIR, 'file' => 'index.html', 'content' => '' ),
		);

		if ( 'redirect' !== $download_method ) {
			$files[] = array( 'base' => $upload_dir['basedir'] . '/evolvewp_erp_uploads', 'file' => '.htaccess', 'content' => 'deny from all' );
		}
	...
	private static function create_options() {
		if ( ! class_exists( 'EvolveWP_ERP_Admin_Settings' ) ) {
			include_once EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'includes/admin/admin-settings.php';
		}
