 ...
 *
 * DEPENDS ON:
 *   - global evolvewp_erp_is_developer_mode() in includes/functions/core.php
 *
 * CONSUMED BY:
 *   - functions.php: evolvewp_erp_log() and evolvewp_erp_trace() global accessors
 *   - EvolveWP_ERP_Enhanced_Logger (via class_exists guard — will update when migrated)
 *   - EvolveWP_ERP_Verification_Logger (via class_exists guard)
 *
 * DATA FLOW:
...
class Logger {
	...
	private function output_trace( $log_entry ) {
		...
		$this->write_log( 'EvolveWP_ERP_Trace: ' . $output );
	}
	...
	private function output_summary() {
		...
		$this->write_log( "EvolveWP_ERP_Summary: Context '{$this->current_context}' - {$total_logs} logs in " . number_format( $duration, 2 ) . 'ms' );
		...
		foreach ( $type_counts as $type => $count ) {
			$this->write_log( "EvolveWP_ERP_Summary: {$type}: {$count}" );
		}

		$loop_summary = $this->get_loop_summary();
		if ( ! empty( $loop_summary ) ) {
			$this->write_log( 'EvolveWP_ERP_Summary: Loops: ' . wp_json_encode( $loop_summary ) );
		}
	}
}
...
add_action( 'admin_footer', function () {
	...
	?>
	<script>
	window.EvolveWPPredictiveERPLogger = {
		context: '',
		loopCounters: {},
		startContext: function(context) {
			this.context = context;
			this.loopCounters = {};
			console.log('EvolveWP_ERP_JS: CONTEXT_START - ' + context);
		},
		trace: function(type, message, data) {
			...
			var output = 'EvolveWP_ERP_JS: [' + type + '] ' + this.context + ' (+' + timestamp.toFixed(2) + 'ms): ' + message;
			if (this.loopCounters[logKey] > 1) { output += ' (x' + this.loopCounters[logKey] + ')'; }
			console.log(output);
			if (Object.keys(data).length > 0) { console.log('EvolveWP_ERP_JS_Data:', data); }
		},
		...
		endContext: function(summary) {
			...
			if (Object.keys(loopSummary).length > 0) { console.log('EvolveWP_ERP_JS_Summary: Loops:', loopSummary); }
			this.context = '';
		}
	};
	</script>
