 ...
 *   - Filter: evolvewp_erp_ecosystem_available_plugins
 *
 * CONSUMED BY:
 *   - Hook: admin_menu (registers the installer submenu page)
 *   - Hook: wp_ajax_evolvewp_erp_install_plugin
 *
 * DATA FLOW:
...
class Installer {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_evolvewp_erp_install_plugin', array( $this, 'ajax_install_plugin' ) );
	}
	...
	private function define_available_plugins() {
		$this->available_plugins = apply_filters( 'evolvewp_erp_ecosystem_available_plugins', array(
			'evolvewp-predictiveerp' => array(
				'name'            => 'EvolveWP PredictiveERP',
				'description'     => 'WordPress plugin boilerplate for the EvolveWP ecosystem.',
				'download_url'    => 'https://github.com/FifeCIC/evolvewp-predictiveerp/archive/main.zip',
				'required_by'     => array(),
	...
	/**
	 * Register the installer submenu page under EvolveWP PredictiveERP settings.
	 *
	 * @since 1.0.0
	 * @return void
	 */
	public function add_installer_page() {
		add_submenu_page(
			'evolvewp-predictiveerp-settings',
			__( 'Install Ecosystem Plugins', 'evolvewp-predictiveerp' ),
			__( 'Install Plugins', 'evolvewp-predictiveerp' ),
			'install_plugins',
			'evolvewp-predictiveerp-ecosystem-installer',
	...
	public function render_installer_page() {
		?>
		<div class="wrap">
			<h1><?php esc_html_e( 'Ecosystem Plugin Installer', 'evolvewp-predictiveerp' ); ?></h1>
			<p><?php esc_html_e( 'Install related EvolveWP plugins with one click.', 'evolvewp-predictiveerp' ); ?></p>
						...
						<div class="plugin-card-top">
							...
							<?php if ( ! empty( $plugin['integrates_with'] ) ) : ?>
								<p class="plugin-integrations">
									<strong><?php esc_html_e( 'Integrates with:', 'evolvewp-predictiveerp' ); ?></strong>
						...
						<div class="plugin-card-bottom">
							...
							<?php elseif ( $is_installed ) : ?>
								<a href="<?php echo esc_url( wp_nonce_url( 'plugins.php?action=activate&plugin=' . $slug, 'activate-plugin_' . $slug ) ); ?>" class="button button-primary">
									<?php esc_html_e( 'Activate', 'evolvewp-predictiveerp' ); ?>
								</a>
							<?php else : ?>
								<button class="button button-primary install-plugin"
										data-slug="<?php echo esc_attr( $slug ); ?>"
										data-name="<?php echo esc_attr( $plugin['name'] ); ?>">
									<?php esc_html_e( 'Install Now', 'evolvewp-predictiveerp' ); ?>
								</button>
							<?php endif; ?>
						</div>
					</div>
	...
	public function ajax_install_plugin() {
		check_ajax_referer( 'evolvewp_erp_install_plugin', 'nonce' );

		if ( ! current_user_can( 'install_plugins' ) ) {
			wp_send_json_error( __( 'Insufficient permissions.', 'evolvewp-predictiveerp' ) );
		}

		if ( empty( $_POST['slug'] ) ) {
			wp_send_json_error( __( 'Plugin slug is required.', 'evolvewp-predictiveerp' ) );
		}

		$slug = sanitize_text_field( wp_unslash( $_POST['slug'] ) );

		if ( ! isset( $this->available_plugins[ $slug ] ) ) {
			wp_send_json_error( __( 'Invalid plugin.', 'evolvewp-predictiveerp' ) );
		}
