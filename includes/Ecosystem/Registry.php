 ...
 *
 * Single responsibility: Maintain a registry of active EvolveWP PredictiveERP-based plugins
 ...
 *
 * CONSUMED BY:
 *   - functions.php  evolvewp_erp_ecosystem() global accessor
 *   - Hook: evolvewp_erp_ecosystem_register (plugins call this to register themselves)
 *
 * DATA FLOW:
 *   Input  → evolvewp_erp_ecosystem_register action (plugin registration calls)
 *   Output → evolvewp_erp_ecosystem_mode option, evolvewp_erp_ecosystem_plugins option
 *
...
/**
 * Manages cross-plugin communication and resource sharing for the EvolveWP PredictiveERP ecosystem.
 *
 * Single responsibility: Track which EvolveWP PredictiveERP-based plugins are active and expose
...
class Registry {
	 ...
	 *
	 * Called by each EvolveWP PredictiveERP-based plugin on the `evolvewp_erp_ecosystem_register`
	...
	public function register_plugin( $slug, $args = array() ) {
		...
		do_action( 'evolvewp_erp_ecosystem_plugin_registered', $slug, $this->registered_plugins[ $slug ] );
	}

	/**
	 * Fire the registration action, check versions, and persist ecosystem state.
	 *
	 * Called on plugins_loaded priority 5. Fires evolvewp_erp_ecosystem_register so
	...
	public function detect_ecosystem() {
		do_action( 'evolvewp_erp_ecosystem_register' );
		$this->check_version_requirements();
		$this->check_plugin_dependencies();
		update_option( 'evolvewp_erp_ecosystem_mode', $this->is_ecosystem_mode() );
		update_option( 'evolvewp_erp_ecosystem_plugins', $this->registered_plugins );
	}
	...
	public function get_menu_location( $default = 'plugin' ) {
		if ( $this->is_ecosystem_mode() ) {
			return get_option( 'evolvewp_erp_ecosystem_menu_location', 'shared' );
		}
		return $default;
	}
						...
						printf(
							...
							esc_html(
								sprintf(
									...
									__( 'requires %s which is not active. Some features may not work correctly.', 'evolvewp-predictiveerp' ),
									$dependency
								)
							)
						);
	...
	private function fire_version_mismatch( $slug, $plugin, $required, $actual ) {
		do_action( 'evolvewp_erp_ecosystem_version_mismatch', $slug, $plugin, $required, $actual );
			...
			printf(
				...
				esc_html(
					sprintf(
						...
						__( 'requires EvolveWP PredictiveERP %1$s or later. Installed version: %2$s. Some features may not work correctly.', 'evolvewp-predictiveerp' ),
						$required,
						$actual ?: __( 'not installed', 'evolvewp-predictiveerp' )
					)
				)
			);
		} );
