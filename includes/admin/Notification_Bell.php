 ...
 * themselves — that is EvolveWP_ERP_Notifications.
 *
 * DEPENDS ON:
 *   - EvolveWP_ERP_Notifications::get_unread_count() (not yet migrated)
 *
 * CONSUMED BY:
 ...
 *
 * DATA FLOW:
 *   Input  → EvolveWP_ERP_Notifications unread count
...
class Notification_Bell {
	...
	public static function add_notification_bell( $wp_admin_bar ) {
		...
		$unread_count = \EvolveWP_ERP_Notifications::get_unread_count( get_current_user_id() );

		$wp_admin_bar->add_node( array(
			'id'    => 'evolvewp-predictiveerp-notifications',
			'title' => self::get_bell_html( $unread_count ),
			'href'  => admin_url( 'admin.php?page=evolvewp-predictiveerp-notifications' ),
			'meta'  => array( 'class' => 'evolvewp-predictiveerp-notification-bell' ),
		) );
	}
	...
	private static function get_bell_html( $count ) {
		...
		if ( $count > 0 ) {
			$html .= '<span class="evolvewp-predictiveerp-notification-count">' . (int) $count . '</span>';
		}
		return $html;
	}
	...
	public static function enqueue_assets() {
		wp_add_inline_style( 'admin-bar', '
			#wp-admin-bar-evolvewp-predictiveerp-notifications .ab-icon { font-size: 20px; margin-top: 2px; }
			.evolvewp-predictiveerp-notification-count { display: inline-block; background: #d63638; color: #fff; border-radius: 10px; padding: 2px 6px; font-size: 11px; font-weight: 600; margin-left: 5px; line-height: 1; }
			#wp-admin-bar-evolvewp-predictiveerp-notifications:hover .ab-icon { color: #00a0d2; }
		' );
	}
}
