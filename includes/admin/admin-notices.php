...
class EvolveWP_ERP_Admin_Notices {
    ...
    */
    ...
    public static function init() {             
        self::$notices = get_option( 'evolvewp_erp_admin_notices', array() );
        add_action( 'evolvewp_erp_installed', array( __CLASS__, 'reset_admin_notices' ) );
        add_action( 'wp_loaded', array( __CLASS__, 'hide_notices' ) );
        add_action( 'shutdown', array( __CLASS__, 'store_notices' ) );

        // Display administrator (staff) only notices.
        if ( function_exists('current_user_can') && current_user_can( 'manage_evolvewp-predictiveerp' ) ) {
    ...
    */    
    public function progress_box( $title, $intro, $progress_array = array() ){    
        echo '
        <div class="evolvewp_erp_status_box_container">
            <div class="welcome-panel">
                ...
                <div class="welcome-panel-content">
                    ...
                    <p>' . esc_html__( 'Pledge £9.99 to the Multitool project for 50% discount on the premium edition.', 'evolvewp-predictiveerp' ) . '</p>                                                     
                </div>
    ...
    public function intro_box( $title, $intro, $info_area = false, $info_area_title = '', $info_area_content = '', $footer = false, $dismissable_id = false ){
        ...
        if ( isset( $_GET[ $dismissable_id ], $_GET['_wpnonce'] ) ) {
            ...
            $evolvewp_erp_dismiss_nonce = sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) );

            if ( 'dismiss' === $_GET[ $dismissable_id ] && wp_verify_nonce( $evolvewp_erp_dismiss_nonce, 'dismiss_notice' ) ) {
        ...
        if( $dismissable_id !== false ) {
            $dismissable_button = sprintf( 
                ' <a href="%s&%s=dismiss&_wpnonce=%s" class="button button-primary"> ' . esc_html__( 'Hide', 'evolvewp-predictiveerp' ) . ' </a>', 
    ...
    */
    ...
    public static function store_notices() {
        update_option( 'evolvewp_erp_admin_notices', self::get_notices() );
    }
    ...
    public static function remove_notice( $name ) {
        ...
        delete_option( 'evolvewp_erp_admin_notice_' . $name );
    }
    ...
    public static function hide_notices() {
        if ( isset( $_GET['evolvewp-predictiveerp-hide-notice'] ) && isset( $_GET['_evolvewp_erp_notice_nonce'] ) ) {
            if ( ! wp_verify_nonce( sanitize_text_field(wp_unslash($_GET['_evolvewp_erp_notice_nonce'])), 'evolvewp_erp_hide_notices_nonce' ) ) {
                wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'evolvewp-predictiveerp' ) );
            }

            if ( function_exists('current_user_can') && ! current_user_can( 'manage_evolvewp-predictiveerp' ) ) {
                wp_die( esc_html__( 'Cheatin&#8217; huh?', 'evolvewp-predictiveerp' ) );
            }

            $hide_notice = sanitize_text_field( wp_unslash($_GET['evolvewp-predictiveerp-hide-notice']) );
            self::remove_notice( $hide_notice );
            do_action( 'evolvewp_erp_hide_' . $hide_notice . '_notice' );
        }
    }
    ...
    public static function add_notices() {
        ...
        if ( ! empty( $notices ) ) {
            wp_enqueue_style( 'evolvewp-predictiveerp-activation', plugins_url(  '/assets/css/activation.css', EVOLVEWP_ERP_PLUGIN_FILE ), array(), EVOLVEWP_ERP_VERSION );
            foreach ( $notices as $notice ) {
                if ( ! empty( self::$core_notices[ $notice ] ) && apply_filters( 'evolvewp_erp_show_admin_notice', true, $notice ) ) {
     ...
     * Add a custom notice.
     * 
     * Example: EvolveWP_ERP_Admin_Notices::add_custom_notice( 'mycustomnotice', 'My name is <strong>Ryan Bayne</strong>' );
     * 
     * @param string $name
    ...
    public static function add_custom_notice( $name, $notice_html ) {
        ...
        update_option( 'evolvewp_erp_admin_notice_' . $name, wp_kses_post( $notice_html ) );
    }
        ...
        if ( ! empty( $notices ) ) {
            foreach ( $notices as $notice ) {
                if ( empty( self::$core_notices[ $notice ] ) ) {
                    $notice_html = get_option( 'evolvewp_erp_admin_notice_' . $notice );
     ...
     * do_update_evolvewp-predictiveerp GET parameter, preventing unauthorised or forged
    ...
    public static function update_notice() {
        if ( version_compare( get_option( 'evolvewp_erp_db_version' ), EVOLVEWP_ERP_VERSION, '<' ) ) {
            $updater = new EvolveWP_ERP_Background_Updater();
            ...
            $evolvewp_erp_do_update = false;
            if ( isset( $_GET['do_update_evolvewp-predictiveerp'], $_GET['_evolvewp_erp_update_nonce'] ) ) {
                $evolvewp_erp_update_nonce = sanitize_text_field( wp_unslash( $_GET['_evolvewp_erp_update_nonce'] ) );
                if ( wp_verify_nonce( $evolvewp_erp_update_nonce, 'evolvewp_erp_do_update' ) && current_user_can( 'manage_options' ) ) {
                    $evolvewp_erp_do_update = true;
                }
            }

            if ( $updater->is_updating() || $evolvewp_erp_do_update ) {
...
add_action('init', array('EvolveWP_ERP_Admin_Notices', 'init'));