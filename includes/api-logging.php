<?php
/**
 * EvolveWP PredictiveERP API Logging
 *
...
class EvolveWP_ERP_API_Logging {
    ...
    public static function ready() {
        $logging_enabled = get_option( 'evolvewp_erp_api_logging_enabled', 'yes' );
        ...
        global $wpdb;
        $table = $wpdb->prefix . 'evolvewp_erp_api_calls';

        // Cache the table-existence result to avoid a direct query on every request.
        $cache_key    = 'evolvewp_erp_api_table_exists_' . $table;
        $table_exists = wp_cache_get( $cache_key, 'evolvewp_erp_api' );
        ...
        if ( false === $table_exists ) {
            ...
            wp_cache_set( $cache_key, $table_exists, 'evolvewp_erp_api', HOUR_IN_SECONDS );
        }
    ...
    public static function log_call($service, $function, $type = 'GET', $status = 'pending', $file = '', $line = '', $description = '', $outcome = '') {
        ...
        $result = $wpdb->insert(
            $wpdb->prefix . 'evolvewp_erp_api_calls',
            $data,
    ...
    public static function track_endpoint($entry_id, $service, $endpoint, $parameters = array()) {
        ...
        $existing = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}evolvewp_erp_api_endpoints WHERE service = %s AND endpoint = %s",
                $service,
                $endpoint
            )
        );
        ...
        if ($existing) {
            $result = $wpdb->update(
                $wpdb->prefix . 'evolvewp_erp_api_endpoints',
                array(
                    'entryid'    => $entry_id,
    ...
    public static function log_error($entry_id, $code, $error, $function = '', $file = '', $line = 0) {
        ...
        $result = $wpdb->insert(
            $wpdb->prefix . 'evolvewp_erp_api_errors',
            array(
                'entryid'   => $entry_id,
    ...
    public static function get_api_calls( $args = array() ) {
        ...
        $cache_key = 'evolvewp_erp_api_calls_' . md5( serialize( $args ) );
        $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_api' );
        ...
        $safe_table = esc_sql( $wpdb->prefix . 'evolvewp_erp_api_calls' );
        ...
        wp_cache_set( $cache_key, $results, 'evolvewp_erp_api', 60 );
    ...
    public static function get_api_call_count( $args = array() ) {
        ...
        $cache_key = 'evolvewp_erp_api_count_' . md5( serialize( $args ) );
        $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_api' );
        ...
        wp_cache_set( $cache_key, $count, 'evolvewp_erp_api', 60 );
