...
class EvolveWP_ERP_Ecosystem_Menu_Manager {
    ...
    public function render_shared_logging() {
        ...
        <div class="wrap">
            ...
            <p><?php esc_html_e('Unified logging across all Ryan Bayne plugins', 'evolvewp-predictiveerp'); ?></p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <button class="button button-small run-now" data-hook="<?php echo esc_attr($hook); ?>">
                                        <?php esc_html_e('Run Now', 'evolvewp-predictiveerp'); ?>
                                    </button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    ...
                    <th><?php esc_html_e('Started', 'evolvewp-predictiveerp'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php
                ...
                if (empty($resources)) {
                    echo '<tr><td colspan="5">' . esc_html__('No background processes running.', 'evolvewp-predictiveerp') . '</td></tr>';
                } else {
                    foreach ($resources as $resource) {
                        if (is_callable($resource['callback'])) {
    ...
    private function render_async_tasks($plugins) {
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                ...
                if (empty($resources)) {
                    echo '<tr><td colspan="4">' . esc_html__('No async tasks queued.', 'evolvewp-predictiveerp') . '</td></tr>';
                } else {
                    foreach ($resources as $resource) {
                        if (is_callable($resource['callback'])) {
                            call_user_func($resource['callback']);
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td>
                            ...
                            <p class="description">
                                <?php esc_html_e('When multiple Ryan Bayne plugins are installed, shared views (logging, CRON, background tasks) can be moved to WordPress Tools and Settings menus.', 'evolvewp-predictiveerp'); ?>
                            </p>
                        </td>
                    </tr>
                </table>
...
return new EvolveWP_ERP_Ecosystem_Menu_Manager();
