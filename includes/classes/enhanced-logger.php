<?php
/**
 * EvolveWP PredictiveERP Enhanced Logger
...
class EvolveWP_ERP_Enhanced_Logger {
    ...
    public function log_query($query) {
        ...
        if (class_exists('EvolveWP_ERP_Unified_Logger')) {
            EvolveWP_ERP_Unified_Logger::instance()->trace('DB_QUERY', "{$query_type} query", array(
                'query_hash' => $query_hash,
    ...
    public function log_hook($hook) {
        ...
        if (class_exists('EvolveWP_ERP_Unified_Logger') && $this->hooks[$hook]['count'] % 50 === 0) {
            EvolveWP_ERP_Unified_Logger::instance()->trace('HOOK_FREQUENT', "Hook {$hook} called {$this->hooks[$hook]['count']} times");
        }
    ...
    public function save_logs() {
        ...
        $table = $wpdb->prefix . 'evolvewp_erp_debug_logs';
        ...
        $table_exists_cache_key = 'evolvewp_erp_logger_table_exists';
        $table_exists = wp_cache_get( $table_exists_cache_key, 'evolvewp_erp_logger' );
        if ( false === $table_exists ) {
            $table_exists = ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $table ) ) === $table );
            wp_cache_set( $table_exists_cache_key, $table_exists, 'evolvewp_erp_logger', 3600 );
        }
        if ( ! $table_exists ) {
            self::create_table();
        }
    ...
    public static function get_recent_logs( $limit = 50 ) {
        ...
        $table     = $wpdb->prefix . 'evolvewp_erp_debug_logs';
        $safe_table = esc_sql( $table );
        $cache_key = 'evolvewp_erp_recent_logs_' . absint( $limit );
        $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_logger' );
        ...
        wp_cache_set( $cache_key, $results, 'evolvewp_erp_logger', 60 );
    ...
    public static function clear_old_logs( $days = 7 ) {
        ...
        wp_cache_delete( 'evolvewp_erp_recent_logs_50', 'evolvewp_erp_logger' );
        wp_cache_delete( 'evolvewp_erp_logger_table_exists', 'evolvewp_erp_logger' );
...
// Initialize
if (EvolveWP_ERP_Developer_Mode::is_dev_environment()) {
    EvolveWP_ERP_Enhanced_Logger::instance();
}

// Create table on activation
register_activation_hook(EVOLVEWP_ERP_PLUGIN_FILE, array('EvolveWP_ERP_Enhanced_Logger', 'create_table'));
