<?php
/**
 * EvolveWP PredictiveERP Library Manager
...
class EvolveWP_ERP_Library_Manager {
    ...
    public function maybe_check_updates() {
        $last_check = get_transient('evolvewp_erp_library_check');
        
        if (false === $last_check) {
            $this->check_all_updates();
            set_transient('evolvewp_erp_library_check', time(), DAY_IN_SECONDS);
        }
    }
    ...
    private function check_library_update($id, $library) {
        ...
        if ($latest && version_compare($latest, $library['version'], '>')) {
            update_option('evolvewp_erp_library_update_' . $id, array(
                'current' => $library['version'],
                'latest' => $latest,
                'checked' => current_time('mysql'),
            ));
        } else {
            delete_option('evolvewp_erp_library_update_' . $id);
        }
    }
    ...
    private function get_latest_version($repo) {
        ...
        $response = wp_remote_get($url, array(
            'timeout' => 10,
            'headers' => array('User-Agent' => 'EvolveWP PredictiveERP-Library-Manager'),
        ));
    ...
    public static function get_library_status($id) {
        ...
        $update = get_option('evolvewp_erp_library_update_' . $id);
        
        // Check if library exists
        $path = EVOLVEWP_ERP_PLUGIN_DIR_PATH . $library['path'] . '/' . $library['file'];
...
// Initialize
EvolveWP_ERP_Library_Manager::instance();
