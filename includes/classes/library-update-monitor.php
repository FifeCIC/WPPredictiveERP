 ...
 */
class EvolveWP_ERP_Library_Update_Monitor {
    ...
    private function __construct() {
        ...
        add_action( 'wp_ajax_evolvewp_erp_dismiss_library_notice', array( $this, 'ajax_dismiss_notice' ) );
    }
    ...
    public function check_updates( $slug ) {
        ...
        $transient_key = 'evolvewp_erp_library_update_' . $slug;
    ...
    public function check_outdated_libraries() {
        ...
        $dismissed = get_user_meta( get_current_user_id(), 'evolvewp_erp_library_notice_dismissed', true );
        ...
        if ( ! empty( $outdated ) ) {
            add_action( 'admin_notices', function() use ( $outdated ) {
                $url = admin_url( 'admin.php?page=evolvewp_erp_development&tab=libraries' );
                echo '<div class="notice notice-warning is-dismissible" data-notice="evolvewp-predictiveerp-library-outdated">';
                echo '<p><strong>EvolveWP PredictiveERP:</strong> ' . esc_html( count( $outdated ) ) . ' bundled ' . esc_html( _n( 'library is', 'libraries are', count( $outdated ), 'evolvewp-predictiveerp' ) ) . ' outdated (12+ months): ' . esc_html( implode( ', ', $outdated ) ) . '</p>';
                echo '<p><a href="' . esc_url( $url ) . '" class="button button-primary">Check for Updates</a> <button type="button" class="button evolvewp-predictiveerp-dismiss-library-notice">Remind Me Later</button></p>';
                echo '</div>';
            } );
        }
    }
    ...
    public function clear_cache( $slug = null ) {
        if ( $slug ) {
            delete_transient( 'evolvewp_erp_library_update_' . $slug );
        } else {
            foreach ( $this->libraries as $slug => $library ) {
                delete_transient( 'evolvewp_erp_library_update_' . $slug );
            }
    ...
    public function ajax_dismiss_notice() {
        update_user_meta( get_current_user_id(), 'evolvewp_erp_library_notice_dismissed', time() );
        wp_send_json_success();
    }
}

// Initialize
EvolveWP_ERP_Library_Update_Monitor::instance();
