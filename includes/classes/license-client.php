<?php
/**
 * EvolveWP PredictiveERP License Client
 * 
...
class EvolveWP_ERP_License_Client {
    
    private $license_server_url;
    private $plugin_slug;
    private $plugin_version;
    private $option_name = 'evolvewp_erp_license_data';
    
    public function __construct() {
        $this->license_server_url = defined('EVOLVEWP_ERP_LICENSE_SERVER') ? EVOLVEWP_ERP_LICENSE_SERVER : '';
        $this->plugin_slug = 'evolvewp-predictiveerp';
        $this->plugin_version = EVOLVEWP_ERP_VERSION;
    ...
    public function check_license_status() {
        ...
        // Check once per day
        $last_check = get_transient('evolvewp_erp_license_check');
        if ($last_check) {
            return;
        }
        ...
        set_transient('evolvewp_erp_license_check', time(), DAY_IN_SECONDS);
    }
    ...
    public function check_for_updates($transient) {
        ...
        if (isset($response['new_version']) && version_compare($this->plugin_version, $response['new_version'], '<')) {
            $plugin_file = plugin_basename(EVOLVEWP_ERP_PLUGIN_DIR_PATH . 'evolvewp-predictiveerp.php');
    ...
    private function api_request($endpoint, $data = array()) {
        ...
        $url = trailingslashit($this->license_server_url) . 'wp-json/evolvewp-predictiveerp-license/v1/' . $endpoint;
    ...
    public function license_notices() {
        ...
        if (empty($license_data)) {
            ...
            echo '<strong>EvolveWP PredictiveERP:</strong> ';
            echo 'No license key activated. <a href="' . esc_url( admin_url('admin.php?page=evolvewp-predictiveerp-license') ) . '">Activate your license</a> to receive updates and premium features.';
            echo '</p></div>';
            return;
        }
        ...
        if (isset($license_data['expires']) && $license_data['expires'] !== 'lifetime') {
            ...
            if ($days_left < 0) {
                ...
                echo '<strong>EvolveWP PredictiveERP:</strong> ';
                echo 'Your license has expired. <a href="' . esc_url( admin_url('admin.php?page=evolvewp-predictiveerp-license') ) . '">Renew your license</a> to continue receiving updates.';
                echo '</p></div>';
            } elseif ($days_left < 30) {
                echo '<div class="notice notice-warning"><p>';
                echo '<strong>EvolveWP PredictiveERP:</strong> ';
                echo 'Your license expires in ' . esc_html( $days_left ) . ' days. <a href="' . esc_url( admin_url('admin.php?page=evolvewp-predictiveerp-license') ) . '">Renew now</a> to avoid interruption.';
                echo '</p></div>';
            }
        }
    }
...
// new EvolveWP_ERP_License_Client();
