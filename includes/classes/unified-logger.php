<?php
/**
 * EvolveWP PredictiveERP Unified Logger
...
class EvolveWP_ERP_Unified_Logger {
...
function evolvewp_erp_js_log_helper() {
    if (!evolvewp_erp_is_developer_mode()) return;
    ?>
    <script>
    window.EvolveWPPredictiveERPLogger = {
        ...
        trace: function(type, message, data = {}) {
            ...
            const output = `EvolveWP_ERP_JS: [${type}] ${this.context} (+${timestamp.toFixed(2)}ms): ${message}`;
            ...
            if (Object.keys(data).length > 0) {
                console.log('EvolveWP_ERP_JS_Data:', data);
            }
        },
        ...
        endContext: function(summary = {}) {
            ...
            if (Object.keys(loopSummary).length > 0) {
                console.log('EvolveWP_ERP_JS_Summary: Loops:', loopSummary);
            }
...
// Initialize
add_action('admin_footer', 'evolvewp_erp_js_log_helper');

// Global helper functions
function evolvewp_erp_log() {
    return EvolveWP_ERP_Unified_Logger::instance();
}

function evolvewp_erp_trace($type, $message, $data = array()) {
    EvolveWP_ERP_Unified_Logger::instance()->trace($type, $message, $data);
}