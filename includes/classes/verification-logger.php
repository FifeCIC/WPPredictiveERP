<?php
/**
 * EvolveWP PredictiveERP Verification Logger
...
class EvolveWP_ERP_Verification_Logger {
     ...
     *
     * @since  1.0.0
     * @return EvolveWP_ERP_Verification_Logger
     */
    ...
    public function log_step( $step_name, $input_count, $output_count, $details = array() ) {
        if ( ! evolvewp_erp_is_developer_mode() ) {
            return;
        }
        ...
        if ( class_exists( 'EvolveWP_ERP_Unified_Logger' ) ) {
            EvolveWP_ERP_Unified_Logger::instance()->php_trace( $step_name, $input_count, $output_count, $details );
        }
        ...
        if ( $step_data['data_loss'] ) {
            $this->write_log( "EvolveWP_ERP_Verification: DATA LOSS in {$step_name}: {$input_count} → {$output_count} (lost {$step_data['loss_amount']})" );
            if ( ! empty( $details ) ) {
                // wp_json_encode() used — handles encoding edge cases and avoids
                // the raw json_encode() call that can silently return false.
                $this->write_log( 'EvolveWP_ERP_Verification: Details: ' . wp_json_encode( $details ) );
            }
        }

        $this->write_log( "EvolveWP_ERP_Verification: {$step_name}: {$input_count} → {$output_count}" );
    }
    ...
    public function log_js_transmission( $operation, $data_count, $payload_size = null ) {
        ...
        $this->write_log(
            "EvolveWP_ERP_Verification: JS_TRACE - {$operation}: {$data_count} items" .
            ( $payload_size ? " ({$payload_size} bytes)" : '' )
        );

        if ( class_exists( 'EvolveWP_ERP_Unified_Logger' ) ) {
            EvolveWP_ERP_Unified_Logger::instance()->js_trace( $operation, $data_count, $details );
        }
    }
     ...
     *
     * Delegates to EvolveWP_ERP_Unified_Logger::loop_trace() which throttles output
    ...
    public function log_file_loop( $loop_id, $file_path, $result_count, $details = array() ) {
        ...
        if ( class_exists( 'EvolveWP_ERP_Unified_Logger' ) ) {
            EvolveWP_ERP_Unified_Logger::instance()->loop_trace( $loop_id, $loop_details );
        }
    }
    ...
    public function log_hash_comparison( $file_path, $old_hash, $new_hash, $changed ) {
        ...
        if ( class_exists( 'EvolveWP_ERP_Unified_Logger' ) ) {
            EvolveWP_ERP_Unified_Logger::instance()->trace(
                'HASH_COMPARE',
    ...
    public function log_payload_extraction( $payload_key, $expected_count, $actual_count, $raw_data = null ) {
        ...
        $this->write_log( "EvolveWP_ERP_Verification: EXTRACT_PAYLOAD - {$payload_key}: expected {$expected_count}, got {$actual_count}" );
    ...
    public function output_summary() {
        ...
        $this->write_log( "EvolveWP_ERP_Verification: SUMMARY - {$summary['total_steps']} steps, {$summary['data_loss_steps']} with data loss" );
        $this->write_log( "EvolveWP_ERP_Verification: SUMMARY - Total data loss: {$summary['total_data_loss']}, Final count: {$summary['final_count']}" );

        foreach ( $summary['steps'] as $step ) {
            $loss_indicator = $step['loss'] > 0 ? " ❌ LOST {$step['loss']}" : ' ✅';
            $this->write_log( "EvolveWP_ERP_Verification: STEP - {$step['step']}: {$step['input']} → {$step['output']}{$loss_indicator}" );
        }
    }
...
function evolvewp_erp_verification_log() {
    return EvolveWP_ERP_Verification_Logger::instance();
}

function evolvewp_erp_log_verification_step($step_name, $input_count, $output_count, $details = array()) {
    EvolveWP_ERP_Verification_Logger::instance()->log_step($step_name, $input_count, $output_count, $details);
}

function evolvewp_erp_log_js_data($operation, $count, $size = null) {
    EvolveWP_ERP_Verification_Logger::instance()->log_js_transmission($operation, $count, $size);
}