<?php
/**
 * EvolveWP PredictiveERP Database Helper Functions
 *
 ...
 */
function evolvewp_erp_db_selectrow( $tablename, $condition, $select = '*' ) {
    global $wpdb;
    ...
    $cache_key = 'evolvewp_erp_row_' . md5( $safe_table . $safe_select . $condition );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
    ...
    wp_cache_set( $cache_key, $result, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_selectwhere( $tablename, $condition = null, $orderby = null, $select = '*', $object = 'ARRAY_A' ) {
    global $wpdb;
    ...
    $cache_key = 'evolvewp_erp_rows_' . md5( $safe_table . $safe_select . $where_clause . $order_clause );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_count_rows( $tablename, $where = '' ) {
    global $wpdb;

    $safe_table = esc_sql( $tablename );
    $where_sql  = empty( $where ) ? '' : ' WHERE ' . $where;

    $cache_key = 'evolvewp_erp_count_' . md5( $safe_table . $where_sql );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
    ...
    wp_cache_set( $cache_key, $count, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_get_value( $column, $tablename, $conditions ) {
    global $wpdb;

    $safe_column = esc_sql( $column );
    $safe_table  = esc_sql( $tablename );

    $cache_key = 'evolvewp_erp_val_' . md5( $safe_column . $safe_table . $conditions );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
    ...
    wp_cache_set( $cache_key, $value, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_table_exists( $table_name ) {
    global $wpdb;

    $cache_key = 'evolvewp_erp_table_exists_' . md5( $table_name );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
    ...
    wp_cache_set( $cache_key, $exists, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_get_table_columns( $table_name, $columns_only = true ) {
    global $wpdb;

    $safe_table = esc_sql( $table_name );
    $cache_key  = 'evolvewp_erp_cols_' . md5( $safe_table . ( $columns_only ? '1' : '0' ) );
    $cached     = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
 ...
 * @return void
 */
function evolvewp_erp_db_drop_table( $table_name ) {
    ...
    wp_cache_delete( 'evolvewp_erp_table_exists_' . md5( $table_name ), 'evolvewp_erp_db' );
}
 ...
 */
function evolvewp_erp_get_table_row_count( $table_name ) {
    global $wpdb;

    $safe_table = esc_sql( $table_name );
    $cache_key  = 'evolvewp_erp_rowcount_' . md5( $safe_table );
    $cached     = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_get_table_size( $table_name ) {
    global $wpdb;

    $cache_key = 'evolvewp_erp_tablesize_' . md5( $table_name );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
    ...
    if ( $table_status && isset( $table_status->data_length, $table_status->index_length ) ) {
        $result = evolvewp_erp_format_bytes( $table_status->data_length + $table_status->index_length );
    } else {
        $result = 'Unknown';
    }
 ...
 */
function evolvewp_erp_format_bytes( $bytes ) {
    if ( $bytes < 1024 ) {
        return $bytes . ' B';
 ...
 */
function evolvewp_erp_get_table_status( $table_name ) {
    global $wpdb;
    ...
    $safe_table = esc_sql( $table_name );
    $cache_key  = 'evolvewp_erp_status_' . md5( $safe_table );
    $cached     = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
 ...
 */
function evolvewp_erp_db_max_value( $column, $tablename ) {
    global $wpdb;

    $safe_column = esc_sql( $column );
    $safe_table  = esc_sql( $tablename );

    $cache_key = 'evolvewp_erp_max_' . md5( $safe_column . $safe_table );
    $cached    = wp_cache_get( $cache_key, 'evolvewp_erp_db' );
