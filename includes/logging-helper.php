<?php
/**
 * EvolveWP PredictiveERP Logging Helper
 * 
...
class EvolveWP_ERP_Logging_Helper {
    
    /**
     * Log general application events
     */
    public static function log_app($message, $context = array()) {
        if (get_option('evolvewp_erp_logging_enabled', 'yes') !== 'yes') {
            return;
        }
    ...
    public static function log_api($action, $endpoint, $details = array()) {
        if (get_option('evolvewp_erp_logging_api', 'yes') !== 'yes') {
            return;
        }
    ...
    public static function log_error($error_code, $error_message, $context = array()) {
        if (get_option('evolvewp_erp_logging_errors', 'yes') !== 'yes') {
            return;
        }
    ...
    public static function log_debug($message, $data = null) {
        if (get_option('evolvewp_erp_logging_debug', 'no') !== 'yes') {
            return;
        }
    ...
    private static function write_log($filename, $message) {
        $log_dir = defined('EVOLVEWP_ERP_LOG_DIR') ? EVOLVEWP_ERP_LOG_DIR : WP_CONTENT_DIR . '/evolvewp-predictiveerp-logs/';
