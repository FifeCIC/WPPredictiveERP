<?php         
/**
 * EvolveWP PredictiveERP - Options Table Interface
 *
 ...
 * @version 1.0 
 */
class EvolveWP_ERP_Options {
    use EvolveWP_ERP_OptionsTrait;
}

trait EvolveWP_ERP_OptionsTrait {
    ...
    */
    private static $grouped_options = array(      
        'compact' => 'evolvewp_erp_options',
        'private' => 'evolvewp_erp_private_options',
    );
    ...
    */
    public static function get_option_names( $type = 'compact' ) {           
        switch ( $type ) {
            ...
            //Individual options here, will be prepended with evolvewp-predictiveerp".
            case 'non_compact' :
                return array(
        ...
        */
        ...
        switch ( $type ) {        
            case 'single':
                ...
                // Remember the real option names are prepend with evolvewp-predictiveerp".
                $single_options = array(  
                    // EvolveWP PredictiveERP core options.                                  
    ...
    */
    ...
    /**
     * Returns the requested option.  Looks in evolvewp_erp_options group 
     * or evolvewp_erp_$name as appropriate.
     *
     * @version 1.2
    ...
    public static function get_option( $name, $default = false, $maybe_unserialize = true ) {
        ...
        if ( self::is_valid( $name, 'non_compact' ) ) {
            $option_value = get_option( "evolvewp_erp_$name", $default );
    ...
    */                               
     ...
     * Updates evolvewp_erp_options or evolvewp_erp_$name as appropriate.
     *
     * @since   1.0.0
     * @version 1.2.0
     *
    ...
    public function update_option( $name, $value, $autoload = null ) {
        if ( self::is_valid( $name, 'non_compact' ) ) {
            ...
            if ( version_compare( $GLOBALS['wp_version'], '4.2', '>=' ) ) {
                return update_option( "evolvewp_erp_$name", $value, $autoload );
            }
            return update_option( "evolvewp_erp_$name", $value );
        }
        ...
        _doing_it_wrong( __METHOD__, sprintf( 'Invalid EvolveWP PredictiveERP option name: %s', esc_html( $name ) ), '1.0.0' );
    ...
    public function update_options( $array ) {
        ...
        foreach ( array_diff( $names, self::get_option_names(), self::get_option_names( 'non_compact' ), self::get_option_names( 'private' ) ) as $unknown_name ) {
            // _doing_it_wrong() integrates with WP_DEBUG and is silenced in production,
            // replacing trigger_error() which can expose file paths.
            _doing_it_wrong( __METHOD__, sprintf( 'Invalid EvolveWP PredictiveERP option name: %s', esc_html( $unknown_name ) ), '1.0.0' );
     ...
     * Updates evolvewp_erp_options and/or deletes evolvewp_erp_$name as appropriate.
     *
     * @since   1.0.0
     * @version 1.2.0
     *
    ...
    public function delete_option( $names ) {
        ...
        if ( ! self::is_valid( $names ) ) {
            // _doing_it_wrong() integrates with WP_DEBUG and is silenced in production,
            // replacing trigger_error() which can expose file paths.
            _doing_it_wrong( __METHOD__, sprintf( 'Invalid EvolveWP PredictiveERP option names: %s', wp_json_encode( $names ) ), '1.0.0' );

            return false;
        }

        foreach ( array_intersect( $names, self::get_option_names( 'non_compact' ) ) as $name ) {
            if ( ! delete_option( "evolvewp_erp_$name" ) ) {
                $result = false;
            }
        }
