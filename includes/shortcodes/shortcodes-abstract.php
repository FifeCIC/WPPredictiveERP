...
if ( ! class_exists( 'EvolveWP_ERP_Shortcode' ) ) :
       
abstract class EvolveWP_ERP_Shortcode extends EvolveWP_ERP_Codec_Instance {
    ...
    public function register ($key) {
        ...
        add_action('evolvewp-predictiveerp-shortcodes-shortcode_help', array($this, "add_shortcode_help"));
        add_filter('evolvewp-predictiveerp-shortcodes-shortcode_help-string', array($this, 'get_shortcode_help'));
    }
    ...
    public function get_shortcode_help ($all='') {             
        ...
        $args_help = '';
        if (!empty($arguments)) {
            $args_help .= '<h4>' . __('Arguments', 'evolvewp-predictiveerp') . '</h4><dl>';
        ...
        $help = '' .
                ...
                sprintf(__('Shortcode <code>%s</code>', 'evolvewp-predictiveerp'), $this->_key) .
                '</span></h3>' .
                '<div class="inside">' .
                '<h4>' . __('Shortcode', 'evolvewp-predictiveerp') . '</h4>' .
                '<pre><code>[' . esc_html($this->_key) . ']</code></pre>' .
                '<h4>' . __('Description', 'evolvewp-predictiveerp'). '</h4>' .
                $usage .
                $args_help .
                '</div>' .
                '</div>' .
    ...
    protected function _to_help_string ($arg, $help) {          
        ...
        if (!empty($help['allowed_values'])) {
            $ret[] = __('Allowed values:', 'evolvewp-predictiveerp') . ' <code>' . join('</code>, <code>', $help['allowed_values']) . '</code>';
        }
        if (!empty($help['example'])) {
            $ret[] = __('Example:', 'evolvewp-predictiveerp') . ' <code>[' . $this->_key . ' ... ' . $arg . '="' . $help['example'] . '"]</code>';
        }
        return join('<br />', $ret);
    }
}
