...
class EvolveWP_ERP_Admin_Toolbar_Developers {
    ...
    private function init() {
        global $wp_admin_bar, $evolvewp_erp_settings;  
        ...
        $args = array(
            ...
            'title'  => '<span class="ab-icon evolvewp-predictiveerp-toolbar-icon"></span><span class="ab-label">' . __( 'WP Seed Developers', 'evolvewp-predictiveerp' ) . '</span>',          
        );
        $wp_admin_bar->add_menu( $args );
        
            // Group - Debug Tools
            $args = array(
                'id'     => 'evolvewp-predictiveerp-toolbarmenu-debugtools',
                'parent' => 'evolvewp-predictiveerp-toolbarmenu-developers',
                'title'  => __( 'Debug Tools', 'evolvewp-predictiveerp' ), 
            ...
            $wp_admin_bar->add_menu( $args );
                ...
                $page_param = ( current_user_can( 'seniordeveloper' ) && isset( $_GET['page'] ) )
                    ...
                    : '';
                $href = wp_nonce_url( admin_url() . 'admin.php?page=' . $page_param . '&evolvewp-predictiveerpaction=' . 'debugmodeswitch'  . '', 'debugmodeswitch' );
                if( !isset( $evolvewp_erp_settings['displayerrors'] ) || $evolvewp_erp_settings['displayerrors'] !== true ) 
                {
                    $error_display_title = __( 'Hide Errors', 'evolvewp-predictiveerp' );
                } 
                else 
                {
                    $error_display_title = __( 'Display Errors', 'evolvewp-predictiveerp' );
                }
                $args = array(
                    'id'     => 'evolvewp-predictiveerp-toolbarmenu-errordisplay',
                    'parent' => 'evolvewp-predictiveerp-toolbarmenu-debugtools',
    ...
    public function add_toolbar_icon_css() {
        ?>
        <style>
            #wpadminbar .evolvewp-predictiveerp-toolbar-icon:before {
                content: '';
