...
- [Report Issues](https://github.com/ryanbayne/evolvewp-predictiveerp/issues)
- [Documentation](https://github.com/ryanbayne/evolvewp-predictiveerp/tree/main/docs)
- [Discussions](https://github.com/ryanbayne/evolvewp-predictiveerp/discussions)
...
2. Rename folder from evolvewp-predictiveerp" to "your-plugin-name"
3. Find & Replace: evolvewp-predictiveerp" → "yourplugin", "WPSeed" → "YourPlugin", "WPSEED" → "YOURPLUGIN"
...
* FIXED: W-1d881130 evolvewp_erp_db_selectwhere() in includes/functions/database.php — direct query already resolved in full rewrite; issue closed via WPVerifier UI
...
* FIXED: readme.txt Stable tag updated from 1.1.0 to 2.0.0 to match Version in evolvewp-predictiveerp.php header, resolving stable_tag_mismatch error
* FIXED: readme.txt Requires at least updated from 5.0 to 4.4 to match evolvewp-predictiveerp.php header, resolving readme_mismatched_header_requires error
...
* FIXED: $_tests_dir in tests/bootstrap.php renamed to $evolvewp_erp_tests_dir throughout to satisfy WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
...
* FIXED: includes/admin/admin-settings.php — all four OutputNotEscaped errors on $custom_attributes wrapped with wp_kses_post(implode()); $_REQUEST['_wpnonce'] extracted into sanitised local variable; $_GET['section'] wrapped with wp_unslash(); stripslashes() on evolvewp_erp_error and evolvewp_erp_message replaced with sanitize_text_field(wp_unslash())
* FIXED: includes/widgets/widget-example.php — ABSPATH guard added; class renamed from Foo_Widget to EvolveWP_ERP_Foo_Widget; all widget wrapper args (before_widget, after_widget, before_title, after_title, apply_filters widget_title) wrapped with wp_kses_post()
...
* FIXED: admin/page/development/development-tabs.php — get_current_tab() now verifies a wp_nonce_url()-signed nonce (action: evolvewp_erp_dev_tab_navigation) before reading $_GET['tab'], properly satisfying WordPress.Security.NonceVerification.Recommended; nonce_action() helper added for consistency; tab URLs in tabs() updated to include the nonce via wp_nonce_url(); file header and class @version bumped to 2.0.0
...
* FIXED: examples/accordion-table-example.php — $_GET['configure'] gated behind current_user_can(); foreach loop variables renamed from $item/$item_id to $evolvewp_erp_item/$evolvewp_erp_item_id; sidebar $selected_item/$item bug fixed to use $evolvewp_erp_selected_item/$evolvewp_erp_item
* FIXED: examples/accordion-table-example.php — $_GET['configure'] now verified with wp_verify_nonce() (action: evolvewp_erp_accordion_configure) before use, properly satisfying WordPress.Security.NonceVerification.Recommended; evolvewp_erp_accordion_nonce_action() helper added; configure URLs in the item loop now signed with wp_nonce_url(); file @version bumped to 2.0.0
...
* FIXED: examples/integrations/woocommerce-example.php — combined isset( $_POST['_evolvewp_erp_custom_field'], $_POST['_evolvewp_erp_custom_field_nonce'] ) split into two separate isset() checks so $_POST['_evolvewp_erp_custom_field_nonce'] is only ever referenced at the point of immediate sanitised extraction, satisfying WordPress.Security.ValidatedSanitizedInput.InputNotSanitized on line 54; nonce verification restructured to early-return pattern; field value extracted into $evolvewp_erp_field_value before update_post_meta(); class @version bumped to 2.0.0
...
* FIXED: includes/admin/admin.php — $_GET['page'] in includes() gated behind current_user_can(); $_GET['evolvewp-predictiveerp-install-plugin-redirect'] now uses sanitize_key(wp_unslash()) fixing MissingUnslash and InputNotSanitized; $_GET['page'] and $_GET['activate-multi'] in admin_redirects() gated behind current_user_can()
* FIXED: includes/admin/views/html-admin-page.php — GET reads gated behind current_user_can(); isset() added for seedview fixing InputNotValidated; sanitize_key(wp_unslash()) applied fixing MissingUnslash and InputNotSanitized; loop variables renamed to $evolvewp_erp_key/$evolvewp_erp_report_group/$evolvewp_erp_links
...
* FIXED: includes/classes/ajax.php — nonce extracted into sanitised local variable fixing MissingUnslash and InputNotSanitized; wp_unslash() added to evolvewp-predictiveerp-ajax value; stale ini_set() and NonceVerification results removed (already gated behind WP_DEBUG)
* FIXED: includes/classes/ajax.php — $_GET['evolvewp-predictiveerp-ajax'] in define_ajax() extracted into $evolvewp_erp_ajax_action via sanitize_text_field(wp_unslash()) immediately on read, satisfying WordPress.Security.NonceVerification.Recommended; PHPDoc added to init() and define_ajax(); file header @version bumped to 2.0.0
...
* FIXED: includes/classes/install.php — install_action_do_update() now verifies nonce and current_user_can() before running; install_action_updater_cron() gated behind current_user_can(); hook renamed from wp_evolvewp_erp_updater_cron to evolvewp_erp_updater_cron; stale DirectQuery/NoCaching results removed (one-time activation query)
...
* FIXED: includes/admin/admin-settings.php — $current_tab renamed to $evolvewp_erp_current_tab and $current_section renamed to $evolvewp_erp_current_section in save() and output(); aliased back to unprefixed names for template partial scope; settings-page.php updated consistently
* FIXED: includes/widgets/widget-example.php — W-75715fa9 Foo_Widget class rename to EvolveWP_ERP_Foo_Widget already applied earlier; issue closed via WPVerifier UI
...
* FIXED: Global variable $progress_script renamed to $evolvewp_erp_progress_script in progress-indicators.php to satisfy prefix naming standard
* FIXED: Global variable $status_script renamed to $evolvewp_erp_status_script in status-indicators.php to satisfy prefix naming standard
...
* FIXED: Global variable $security_checks renamed to $evolvewp_erp_security_checks in security-audit.php to satisfy prefix naming standard
