<?php
/**
 * Uninstall EvolveWP PredictiveERP.
 *
 ...
 * @author      Ryan Bayne
 * @category    Core
 * @package     EvolveWP PredictiveERP/Uninstaller
 * @version     2.0.0
 */
...
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_evolvewp_erp_' ) . '%'
    )
);
$wpdb->query(
    $wpdb->prepare(
        "DELETE FROM {$wpdb->options} WHERE option_name LIKE %s",
        $wpdb->esc_like( '_transient_timeout_evolvewp_erp_' ) . '%'
    )
);
...
wp_clear_scheduled_hook( 'evolvewp_erp_daily_cleanup' );
